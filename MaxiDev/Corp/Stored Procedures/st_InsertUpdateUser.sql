CREATE Procedure [Corp].[st_InsertUpdateUser]    
(    
@IdUser Int,    
...
IF Exists(Select 1 from users with(nolock) where (LTRIM(RTRIM(UPPER(UserLogin))) <> LTRIM(RTRIM(UPPER(@UserLogin))) AND IdUser = @IdUser) OR @IdUser = 0) 
BEGIN
	If Exists(Select 1 from users with(nolock) where LTRIM(RTRIM(UPPER(UserLogin)))=LTRIM(RTRIM(UPPER(@UserLogin))))    
	Begin    
		Set @HasError=1      
...
Insert into @UserEdit (Field, Change) (Select 'Status', GenericStatus from GenericStatus with(nolock) where IdGenericStatus = @IdGenericStatus);
...
Insert into @UserEdit values ('Address', @oldAddress);
...
Insert into @UserEdit values ('Seller Parent', (Select UserName from Users with(nolock) where IdUser=@IdUserSellerParent));
 ...
 if @IdUserType=3    
  Begin    
     Update Seller set    
           ...
           Cellular=@Cellular,    
		   Email=@Email,		
           IdUserSellerParent=NULLIF(@IdUserSellerParent, 0) ,
           IdCounty=@IdCounty   
           Where IdUserSeller=@IdUser    
  End
--Guarda los viejos permisos
insert into @OldOptions (idAction)
(Select idAction from ActionAllowed AA with(nolock) join 
(Select OpU.IdOption, s.Item from OptionUsers OpU with(nolock) outer apply dbo.fnSplit(OpU.Action, '|') s where IdUser = @IdUser) OpA
...
begin
	...
	,[Field]
	,[Change])
	(Select @idUserEdit, @IdUser, @CurrentDate, Field, isnull(Change, '') from @UserEdit)
...
Declare @canDeleteSP int = 0
 ...
 IF EXISTS(SELECT 1 FROM OptionUsers with(nolock) WHERE IdUser = @EnterByIdUser AND IdOption IN (SELECT IdOption FROM [Option] with(nolock) WHERE NAME = 'UsersPermissionsCp' OR NAME = 'UsersPermissionsAg')  AND Action like '%SP%')
	BEGIN
		SET @canDeleteSP = 1
	END
...
ELSE
	BEGIN
		 Delete OptionUsers where IdUser=@IdUser and IdOption not in (SELECT IdOption FROM [Option] with(nolock) WHERE NAME = 'UsersPermissionsCp' OR NAME = 'UsersPermissionsAg')
	END
...
Begin Catch      
 ...
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('[Corp].[st_InsertUpdateUser]',Getdate(),@ErrorMessage)      
End Catch
