CREATE PROCEDURE [Corp].[st_UpdateProductTransferStatus_Operation]
(
...
select @Note=Statusname from [status] with(nolock) where idstatus=@IdStatus
   ...
   exec [Corp].[st_SaveChangesToProductTransferLog_Operation]
		@IdProductTransfer = @IdProductTransfer,
		@IdStatus = @IdStatus,
		@Note = @Note,
...
Begin Catch                                                                                        
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_UpdateProductTransferStatus_Operation',Getdate(),@ErrorMessage)  ;                                                                                          
End Catch
