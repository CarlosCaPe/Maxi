CREATE PROCEDURE [Corp].[st_OfacAuditForAgent]
@ExecutionIdUser int,
...
begin
	...
	(
		...
		union
			select A.ComplianceOfficerName Name,
				'' FirstLastName,
				'' SecondLastName,
				8 [Type],	--Agent
                A.AgentCode,
                a.idagent IdGeneric
			from dbo.Agent A with (nolock)
				where A.IdAgentStatus!=@IdAgentStatusDisable and A.IdAgentStatus!=@IdAgentStatusWOff and A.IdAgent=@IdAgent and @IDtype=8
		/*
		 union
	...
	Begin
        ...
        if (@IdOfacAuditType in (6,7,8))
        begin
		    insert into @MatchDetail
		    exec [Corp].[ST_OFAC_SEARCH_DETAILS] @Name,@FirstLastName,@SecondLastName;
        end
        else
        begin
            ...
            begin
                ...
                insert into @MatchDetail
                SELECT SDN_NAME, ISNULL(REMARKS,'') SDN_REMARKS,ISNULL(ALT_TYPE,'') ALT_TYPE,ISNULL(ALT_NAME,'') ALT_NAME, ISNULL(ALT_REMARKS,'') ALT_REMARKS,isnull(ADDRESS,'') ADD_ADDRESS,isnull(CITY_NAME,'') ADD_CITY_NAME,isnull(COUNTRY,'') ADD_COUNTRY,isnull(ADD_REMARKS,'') ADD_REMARKS FROM OFAC_SDN with(nolock)
...
begin catch
	...
	insert into dbo.ErrorLogForStoreProcedure(StoreProcedure, ErrorDate, ErrorMessage) Values('[Corp].[st_OfacAuditForAgent]', GETDATE(), @errormessage)
end catch
