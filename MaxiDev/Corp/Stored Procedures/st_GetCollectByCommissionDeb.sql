CREATE PROCEDURE [Corp].[st_GetCollectByCommissionDeb]
(
    @BeginDate DATETIME,
    @EndDate DATETIME,
...
<ChangeLog>
<log Date="16/01/2018" Author="jmolina">Se agrega la tabla temporal #AgentMirror y se llena con over partition by idagent</log>
...
*********************************************************************/
	...
	SELECT IdAgent, 0, IdAgentCommissionPay, IdAgentPaymentSchema FROM Agent WITH(NOLOCK) WHERE IdAgentStatus IN (1,2,3,4,7) 
	...
	CREATE UNIQUE CLUSTERED INDEX IX_TMP_AgentMirror_IdAgent ON #AgentMirror (idAgent)
		 ...
		 FROM #pivotAgent p 
		INNER JOIN Agent a WITH(NOLOCK) ON a.IdAgent = p.idagent
	...
	IF @CommissionFilter <= 0
		SELECT [IdAgent], [AgentClass], [AgentCode], [AgentName], [RetainMoneyCommission], [Commission], [SpecialCommission], [CommissionRetain], [Debit], [BonusDebit], [SpecialCommApplied] FROM #temporalTable ORDER BY [AgentCode] ASC
	IF @CommissionFilter = 1
		...
		WHERE ([Commission] - [CommissionRetain] - [SpecialCommission]) <= 0 ORDER BY [AgentCode] ASC
