CREATE PROCEDURE [Corp].[st_CheckUpdateVerifyHold]
 (
    @EnterByIdUser INT, 
...
********************************************************************/
			...
			BEGIN
				EXEC [Corp].[st_SENDKYCCheckMessage_msg]
					...
					BEGIN
						...
						EXEC [Corp].[st_RejectCheckNotification] @IdCheck, @EnterByIdUser, @Note
					END
			END
			ELSE
			BEGIN
	...
	BEGIN CATCH
		...
		INSERT  INTO ErrorLogForStoreProcedure (StoreProcedure, ErrorDate, ErrorMessage)Values('[Corp].[st_CheckUpdateVerifyHold]', GETDATE(), @ErrorMessage)
	END CATCH
