CREATE procedure [Corp].[st_GetValidationRules_BillPayment]( @idState int,@IdValidationRule int)
as
/********************************************************************
...
select
 ...
 into #temp2	
from Billpayment.EntityToValidate EV with(nolock)
	inner join Billpayment.ValidationRules VR with(nolock) on VR.IdEntityToValidate =EV.IdEntityToValidate
	inner join Billpayment.Validator V with(nolock) on V.IdValidator=VR.IdValidator
	left join Billpayment.LengthRule LR with(nolock) on LR.IdValidationRule= VR.IdValidationRule
	left join Billpayment.RangeRule RR with(nolock) on RR.IdValidationRule =VR.IdValidationRule
	left join Billpayment.RegularExpressionRule ER with(nolock) on ER.IdValidationRule= VR.IdValidationRule
	left join Billpayment.SimpleComparisonRule CR with(nolock) on CR.IdValidationRule = VR.IdValidationRule
