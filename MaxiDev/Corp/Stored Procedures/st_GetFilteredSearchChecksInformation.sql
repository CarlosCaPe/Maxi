CREATE PROCEDURE [Corp].[st_GetFilteredSearchChecksInformation]
...
@OnlyNotIRDProcessed BIT = 0,
@OnlyIRDToPrint BIT = 0,
...
<log Date="2020/10/16" Author="esalazar" Name="">-- CR M00207	</log>
<log Date="2022/12/07" Author="cagarcia">M1-639 Mostrar solo cheques que tengan CanReprintCheck = 1 cuando se consulten IRDs a imprimir</log>
</ChangeLog>
...
SET ARITHABORT ON;   
    ...
    [IdOccupation] int, /*M00207*/
	[IdSubcategoryOccupation] int ,/*M00207*/
    ...
    BEGIN
		...
		WHERE (CH.IdAgent = @IdAgent OR isnull(@IdAgent, 0) = 0)
			AND CAST(CH.DateOfMovement AS DATE) >= CASE WHEN @FechaInicio IS NULL THEN CAST(CH.DateOfMovement AS DATE) ELSE @FechaInicio END
			AND CAST(CH.DateOfMovement AS DATE) <= CASE WHEN @FechaFin IS NULL THEN CAST(CH.DateOfMovement AS DATE) ELSE @FechaFin END
			AND RR.CanRePrintCheck = 1
			
		)
		
		INSERT INTO #TempIRD
		SELECT t1.IdCheck FROM t1
		WHERE 1=1
			AND (t1.CheckNumber + t1.RoutingNumber + t1.Account) not in (select CheckNumber + RoutingNumber + Account 
				from t1 
				group by CheckNumber, RoutingNumber, Account 
				having count (*) > 1)
		...
		INSERT INTO #TempChecks
				SELECT TOP 2001
					...
					, [C].[CountryBirthId]
					, [C].[Ocupation]
					,ISNULL([CUST].IdOccupation,0) as IdOccupation
					,ISNULL([CUST].IdSubcategoryOccupation,0) as IdSubcategoryOccupation
					,ISNULL([CUST].SubcategoryOccupationOther,'') as SubcategoryOccupationOther
					, [C].[IdCustomer]
					, [C].[DateOfIssue]
					...
					CASE WHEN [C].IdStatus = 31 then [C].DateStatusChange END DateRejected
					--,[C].[IdCustomer]
				FROM
				[dbo].[Checks] AS [C] WITH (NOLOCK)
				INNER JOIN #TempIRD IRD ON IRD.Idcheck = C.IdCheck
				...
				INNER JOIN [dbo].Customer AS [CUST] WITH (NOLOCK) ON [CUST].IdCustomer = [C].IdCustomer
				...
				WHERE [C].[IdStatus] != 41 -- Verify Hold	
