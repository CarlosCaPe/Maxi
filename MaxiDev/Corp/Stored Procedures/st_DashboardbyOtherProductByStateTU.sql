CREATE PROCEDURE [Corp].[st_DashboardbyOtherProductByStateTU]
(    
    @WeeksAgo int,
...
set @OtherProductPrefix='TU'
set @IdOtherProduct=6
...
Select SUM(NumTran) as TotalWeeksAgo, IdAgent into #T1
	from(
		select SUM(1) as NumTran, A.IdAgent from PureMinutesTopupTransaction T
...
Select SUM(NumTran) as TotalToday, sum(AmountInDollars) TotalAmountInDollarsToday, IdAgent into #T2
	from(
		select SUM(1) as NumTran,sum(corpcommission) AmountInDollars, A.IdAgent from PureMinutesTopupTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
