CREATE PROCEDURE [Corp].[st_DashboardbyOtherProductTU]
(
    @WeeksAgo int,
...
Select SUM(NumTran) as TotalWeeksAgo, AgentState into #T1
	from(
		select SUM(1) as NumTran, A.AgentState from PureMinutesTopupTransaction T 
...
Select SUM(NumTran) as TotalToday,sum(AmountInDollars) TotalAmountInDollarsToday, AgentState into #T2
	from(
		select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from PureMinutesTopupTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
