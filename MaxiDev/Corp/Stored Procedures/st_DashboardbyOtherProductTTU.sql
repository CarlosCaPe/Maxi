CREATE PROCEDURE [Corp].[st_DashboardbyOtherProductTTU]
(    
    @WeeksAgo int,
...
Select SUM(NumTran) as TotalWeeksAgo, AgentState into #T1
	from(
		select SUM(1) as NumTran, A.AgentState from TransferTo.[TransferTTo] T 
		...
		select SUM(1)*-1 as NumTran, A.AgentState from TransferTo.[TransferTTo] T
		Join Agent A on (T.IdAgent=A.IdAgent)
...
Select SUM(NumTran) as TotalToday,sum(AmountInDollars) TotalAmountInDollarsToday, AgentState into #T2
	from(
		select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from TransferTo.[TransferTTo] T
		Join Agent A on (T.IdAgent=A.IdAgent)
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from TransferTo.[TransferTTo] T
		Join Agent A on (T.IdAgent=A.IdAgent)        
