CREATE PROCEDURE [Corp].[st_GetGroupByClass_Collection]
as
select t.IdAgentClass,t.IsSpecial,AgentClass,isnull(tot,0) TOT from (
select idagentclass,0 IsSpecial,Description AgentClass from AgentClass with(nolock)
union all
select idagentclass,1 IsSpecial,'Special Category ' + Description AgentClass from AgentClass with(nolock)
) t
left join
(
	SELECT 
	...
	JOIN
		AgentClass C with(nolock) ON G.IdAgentClass=C.IdAgentClass
	Where
		g.IdGenericStatus=1
	GROUP BY 
