CREATE Procedure [Corp].[st_DenyListBeneficiaryMatch]  
(  
...
Select B.Action ,A.MessageInEnglish ,A.MessageInSpanish  from DenyListBeneficiaryActions A WITH(NOLOCK) Join KYCAction B WITH(NOLOCK) ON (A.IdKYCAction=B.IdKYCAction)
Where IdDenyListBeneficiary in
(
Select 
A.IdDenyListBeneficiary                
From dbo.DenyListBeneficiary A WITH(NOLOCK) JOIN Beneficiary B WITH(NOLOCK) ON (A.IdBeneficiary=B.IdBeneficiary)
