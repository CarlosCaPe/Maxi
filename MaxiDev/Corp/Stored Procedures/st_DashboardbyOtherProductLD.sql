CREATE PROCEDURE [Corp].[st_DashboardbyOtherProductLD] /*le agregue el el where 'and t.ReceiveAmount>0'*/
(    
    @WeeksAgo int,
...
set @OtherProductPrefix='LD'
set @IdOtherProduct=5
...
Select SUM(NumTran) as TotalWeeksAgo, AgentState into #T1
	from(
		select SUM(1) as NumTran, A.AgentState from PureMinutesTransaction T 
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransaction>= @StartDate and T.DateOfTransaction<@EndDate and t.status in (select id from @TransactionStatusForPositive)
		...
		select SUM(1)*-1 as NumTran, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.CancelDateOfTransaction>= @StartDate and T.CancelDateOfTransaction<@EndDate and t.status in (select id from @TransactionStatusForNegative)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState
...
Select SUM(NumTran) as TotalToday,sum(AmountInDollars) TotalAmountInDollarsToday, AgentState into #T2
	from(
		select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransaction>= @NowStart and T.DateOfTransaction<@Now and t.status in (select id from @TransactionStatusForPositive)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDateOfTransaction>= @NowStart and T.CancelDateOfTransaction<@Now and t.status in (select id from @TransactionStatusForNegative)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
...
Select SUM(NumTran) as TotalOneMonthAgo,sum(AmountInDollars) TotalAmountInDollarsOneMonthAgo, AgentState into #T3 
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransaction>= @OneMonthAgoSD and T.DateOfTransaction<@OneMonthAgoED and t.status in (select id from @TransactionStatusForPositive)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDateOfTransaction>= @OneMonthAgoSD and T.CancelDateOfTransaction<@OneMonthAgoED and t.status in (select id from @TransactionStatusForNegative)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
...
Select SUM(NumTran) as TotalTwoMonthAgo,sum(AmountInDollars) TotalAmountInDollarsTwoMonthAgo, AgentState into #T4 
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransaction>= @TwoMonthAgoSD and T.DateOfTransaction<@TwoMonthAgoED and t.status in (select id from @TransactionStatusForPositive)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDateOfTransaction>= @TwoMonthAgoSD and T.CancelDateOfTransaction<@TwoMonthAgoED and t.status in (select id from @TransactionStatusForNegative)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
...
Select SUM(NumTran) as TotalThreeMonthAgo,sum(AmountInDollars) TotalAmountInDollarsThreeMonthAgo, AgentState into #T5 
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransaction>= @ThreeMonthAgoSD and T.DateOfTransaction<@ThreeMonthAgoED and t.status in (select id from @TransactionStatusForPositive)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDateOfTransaction>= @ThreeMonthAgoSD and T.CancelDateOfTransaction<@ThreeMonthAgoED and t.status in (select id from @TransactionStatusForNegative)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
...
Select SUM(NumTran) as TotalCurrentMonth,sum(AmountInDollars) TotalAmountInDollarsCurrentMonth, AgentState into #T6
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransaction>= @CurrentMonthSD and T.DateOfTransaction<@CurrentMonthED and t.status in (select id from @TransactionStatusForPositive)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from PureMinutesTransaction T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDateOfTransaction>= @CurrentMonthSD and T.CancelDateOfTransaction<@CurrentMonthED and t.status in (select id from @TransactionStatusForNegative)
		and t.ReceiveAmount>0
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
