CREATE PROCEDURE [Corp].[st_SaveTicket]
(
...
<Description>Crear o Editar Ticket</Description>

<ChangeLog>
<log Date="17/01/2020" Author="jzuniga">BGF154: Cambio de GETDATE() por parametro @OperationDate</log>
</ChangeLog>
********************************************************************/
		...
		IF (@IdTicket = 0)
			BEGIN
				...
				VALUES (GETDATE(), @IdUser, @IdProduct, @IdTransaction, @Note, @IdStatus, @IdPriority, @IdTicketCloseReason, @LastChange_LastUserChange, GETDATE(), 
					@LastChange_LastIpChange, @LastChange_LastNoteChange, @Reference, @OperationDate, @IdAgent, @ClosedDate) --BGF154
