CREATE PROCEDURE [Corp].[st_SavePayerConfigCommissionXML]
(
...
while exists (select top 1 1 from #PayerConfigCommission)
    ...
    exec [Corp].[st_SavePayerConfigCommission] @IdPayerConfig,@Date,@IdUser,@OldValue,@NewValue
...
Begin Catch
 ...
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('[Corp].[st_SavePayerConfigCommissionXML]',Getdate(),ERROR_MESSAGE())    
End Catch
