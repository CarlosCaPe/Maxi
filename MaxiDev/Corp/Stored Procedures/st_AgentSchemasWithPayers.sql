CREATE PROCEDURE [Corp].[st_AgentSchemasWithPayers]
(
    @IdAgent int,
    @IdLenguage int = null   
)   
...
SELECT *
FROM (
		SELECT orderS, CountryName, IdAgentSchema,SchemaName,IdCurrency,CurrencyCode,CurrencyName,IdPaymentType,PaymentName,IdPayer,PayerCode,PayerName,SchemaTempSpreadValue,PayerSpreadValue,SchemaSpreadValue,SchemaIdSpread,RefExRate,IdPayerConfig,IdCountry,IdFee, MaxSpreadDetail,
		...
		FROM (
				SELECT 
				   orderS, IdAgentSchema,SchemaName,IdCurrency,CurrencyCode,CurrencyName,IdPaymentType,PaymentName,IdPayer,PayerCode,
					CASE WHEN G.IDGROUP IS NULL THEN PayerName ELSE G.name END PayerName,
					SchemaTempSpreadValue,PayerSpreadValue,SchemaSpreadValue,SchemaIdSpread,RefExRate,R.IdPayerConfig,IdCountry, IdFee,  CountryName
					, MaxSpreadDetail --29012019-azavala
