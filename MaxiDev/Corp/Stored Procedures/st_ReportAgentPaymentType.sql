CREATE PROCEDURE [Corp].[st_ReportAgentPaymentType] 
...
--cagarciag, 2021-06-01: Se obtienen subestatus de suspension
SELECT *
INTO #tmpSuspendedAgentDetail
FROM
(
SELECT IdAgent, max(CASE WHEN IdMaxiDepartment = 1 THEN convert(INT, Suspended) END) SuspendedByCompl,
	max(CASE WHEN IdMaxiDepartment = 2 THEN convert(INT, Suspended) END) SuspendedByAML,
	max(CASE WHEN IdMaxiDepartment = 3 THEN convert(INT, Suspended) END) SuspendedByAR,
	max(CASE WHEN IdMaxiDepartment = 4 THEN convert(INT, Suspended) END) SuspendedByFM,
	max(CASE WHEN IdMaxiDepartment = 5 THEN convert(INT, Suspended) END) SuspendedByAA 
FROM Corp.AgentSuspendedSubStatus
GROUP BY IdAgent
) A
WHERE A.SuspendedByCompl = 1 OR A.SuspendedByAML = 1 OR A.SuspendedByAR = 1 OR A.SuspendedByFM = 1 OR A.SuspendedByAA = 1
...
if @IdAgentCommissionPay=0
    ...
    select AgentCode,AgentName,AgentStatus,AgentCommissionPayName,AgentCurrentStatusName,AgentCurrentCommissionPayName,Commissions,DebCommissions,AgentPayments,TotalCollectPlan,TotalDebt, SuspendedByAR from
    (
    SELECT	A.AgentCode AS AgentCode, 
			A.AgentName AS AgentName, 
            ...
            CASE WHEN AST.IdAgentStatus = 3 AND S.SuspendedByAR = 1 THEN 'Yes'
            	WHEN AST.IdAgentStatus = 3 AND S.SuspendedByAR = 0 THEN 'No'
            	WHEN AST.IdAgentStatus <> 3 THEN '' END AS 'SuspendedByAR'
	FROM	Agent AS A with(nolock)
            ...
            LEFT JOIN #tmpSuspendedAgentDetail AS S ON S.IdAgent = A.IdAgent
