CREATE PROCEDURE [Corp].[st_UdateAgentCreditLimit]
(
    @IdAgent int,
    @CreditAmount money,
    @EnterbyIdUser int,
...
if isnull(@CreditAmount,0)>0 
   begin
		if(ISNULL((select creditamount from agent with(nolock) where idagent = @IdAgent),0) != @CreditAmount)
			begin
...
EXEC [Corp].[st_SaveAgentCreditLimitHistory]  @IdAgent, @CreditAmount, @EnterbyIdUser, @Note, @HasError
