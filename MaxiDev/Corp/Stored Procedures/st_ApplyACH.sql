CREATE PROCEDURE [Corp].[st_ApplyACH]
(
    @ACHDate DATE,
    @IdUser INT,
...
SELECT IdACHMovement,m.idagent,amount,referenceamount,ismanual,ISNULL(b.bankname,'') bankname,m.Note 
    FROM dbo.ACHMovement m WITH(NOLOCK)
JOIN 
    agent a WITH(NOLOCK) ON m.idagent=a.idagent
LEFT JOIN agentbankdeposit b WITH(NOLOCK) ON a.IdAgentBankDeposit=b.IdAgentBankDeposit
LEFT JOIN(
