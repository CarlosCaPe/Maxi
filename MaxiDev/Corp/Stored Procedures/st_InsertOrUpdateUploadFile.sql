CREATE PROCEDURE [Corp].[st_InsertOrUpdateUploadFile]
(
    @Id INT,
...
<ChangeLog>
    <log Date="2022/04/04" Author="jcsierra"> Se agrega IF para permitir agregar registros con @FileName, cuanto el IdType = 7 </log>
</ChangeLog>
...
DECLARE @newName NVARCHAR(MAX)
	...
	BEGIN
        ...
        IF EXISTS (SELECT 1 FROM DocumentTypes dt WHERE dt.IdDocumentType = @IdDocumentType AND dt.IdType = 7)
        BEGIN
            SET @newName = @FileName
        END
        ELSE IF(@IdDocumentType != @IdDocumentTypeFax)
		BEGIN
...
SET @HasError=1
    ...
    INSERT INTO ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)VALUES('[Corp].[st_InsertOrUpdateUploadFile]',GETDATE(),@ErrorMessage) 
