CREATE PROCEDURE [Corp].[st_MoveTransferReturn]
(
	@IdTransfer int,
    @EnterByIdUser int,
...
Begin Try
	...
	begin -- se cambia a status 41
		...
		BEGIN
			...
			exec [Corp].[st_SimpleAddNoteToTransfer] @IdTransfer, @Note;
		END
	end
	else
	...
	exec [Corp].[st_AddNoteToTransfer] @IdTransfer, @EnterByIdUser, @Note, @IsSpanishLanguage, @TransferDetail out, @HasErrorNote out, @MessageNote out;
...
Begin Catch
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_MoveTransferReturn',Getdate(),@ErrorMessage)
End Catch
