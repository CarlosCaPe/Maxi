Create Procedure [Corp].[st_CreateMessageForAgent_msg]
(
        @IdAgent int,		
...
FROM 
    dbo.Agent a WITH(NOLOCK)
left join 
    agentcurrentbalance b WITH(NOLOCK) on a.idagent=b.idagent
...
BEGIN   
    ...
    IF NOT EXISTS (
        SELECT TOP 1 1 FROM AgentNotificacionReminder n WITH(NOLOCK)
        join msg.[MessageSubcribers] s WITH(NOLOCK) on n.idmessage=s.idmessage
    ...
    BEGIN
        ...
        BEGIN
            ...
            EXEC [Corp].[st_DismissNotificationReminder]
		        @IdAgent,
		        @IsSpanishLanguage,
		        @HasError2 OUTPUT,
		        @MessageOut2 OUTPUT
