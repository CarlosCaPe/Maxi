CREATE PROCEDURE [Corp].[st_DepositCollectionNotification]
    @IdAgent Int
    ,@Deposit Money
...
BEGIN
    ...
    Begin try
	   ...
	   BEGIN
		  ...
		  BEGIN
			 ...
			 /*Cuando lleguen al 70% y menos del porcentaje del que se liberarían de suspendidas, de acuerdo a su categoría (A, B, C y D) de su monto a depositar*/
			 ...
			 /*Porcentaje faltante para alcanzar el mínimo requerido.*/
			 ...
			 BEGIN
				/*>=50% And <70%*/
				IF EXISTS(SELECT TOP 1 1 FROM CollectionNotificationDeposit WITH (NOLOCK)
							 WHERE IdAgent = @IdAgent
			 ...
			 BEGIN
				...
				ELSE
				    SELECT @recipients = 'sherrera@boz.mx; laguilar@boz.mx; soportemaxi@boz.mx '; /*; mmendoza@maxi-ms.com*/
	...
	begin catch
		...
		Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('[Corp].[st_DepositCollectionNotification]',Getdate(),'Agent:'+Convert(VARCHAR(250),@IdAgent)+','+@ErrorMessage);
	End Catch
