CREATE PROCEDURE [Corp].[st_GetGatewayConfig]
(
	@IdStatus int
)
as
...
select @subject = Value from GlobalAttributes with(nolock) where Name like @SubjectKey
select @Note = Value from GlobalAttributes with(nolock) where Name like @NoteKey

select @IdStatus as 'IdStatus', isnull(@subject, '') as 'Subject', isnull(@Note, '') as 'Note'

select gc.IdGatewayConfigMail as 'IdGatewayConfig', gc.Mail, gc.IdGateway, g.GatewayName from GatewayConfigMail gc with(nolock)
join Gateway g with(nolock) on gc.IdGateway = g.IdGateway
