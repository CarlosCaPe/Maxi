CREATE PROCEDURE [Corp].[st_saveBranchesBatch] (@idPayer INT , @IdGateway INT, @EnterByIdUser INT, @data xml, @HasError BIT = 0 OUT,@MessageOUT VARCHAR(200) ='' OUT)
/********************************************************************
...
AS BEGIN
	...
	BEGIN TRY
	   ...
	   HAVING count(*) > 1
		...
		AND NOT EXISTS (SELECT 1 FROM GatewayBranch gb WITH (NOLOCK) WHERE gb.IdGateway =@idGateway AND gb.IdBranch = x.idBranch)
	...
	BEGIN CATCH 
	   ...
	   Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('[Corp].[st_saveBranchesBatch]',Getdate(),@ErrorMessage)   
	  set @HasError =1  
