CREATE PROCEDURE [Corp].[st_ApplyReconcilePayments]
(
    @DataXml XML,
    @IdAgent INT,
	@IdUser INT,
...
Begin
	...
	Begin
        ...
        if (isnull(@BankName,'')='')
        begin
            select @BankName = BankName from agentbankdeposit with(nolock) where IdAgentBankDeposit in(
            select IdAgentBankDeposit from agent with(nolock) where idagent=@IdAgent)
        end
		...
		FROM dbo.AgentDeposit d with(nolock)
		JOIN AgentCollectType c with(nolock) ON d.IdAgentCollectType =c.IdAgentCollectType
		JOIN dbo.Users u with(nolock) ON d.EnterByIdUser=u.IdUser
		WHERE 
			IdAgent=@IdAgent AND
...
Begin transaction	
	...
	Begin
        ...
        exec [Corp].[st_ApplyDepositMaxicollectionForWeekend] @idagent,@maxicollectiondate,@TempAmount
        ...
        Begin
            ...
            --select @recipients = 'vlara@boz.mx; aduran@boz.mx; jvelarde@boz.mx'
