CREATE PROCEDURE [Corp].[st_SaveDeposit]
...
<log Date="30/03/2023" Author="jfresendiz">BM-518 Se añade TypeMovement para separar los Advance Credit </log>
</ChangeLog>
*********************************************************************/
	...
	BEGIN TRY
		...
		--	ELSE
		--		SELECT @recipients = 'jvelarde@boz.mx; soportemaxi@boz.mx; mmendoza@maxi-ms.com'
		...
		--		*/
		  ...
		  EXEC [Corp].[st_DepositCollectionNotification] @IdAgent, @Amount, @CollectDate, @HasError OUTPUT, @Message OUTPUT;
		...
		BEGIN
			...
			BEGIN
				SELECT @EmailProfile = [Value] FROM [dbo].[GlobalAttributes] WITH (NOLOCK) WHERE [Name] = 'EmailProfiler'
				INSERT INTO [dbo].[EmailCellularLog] VALUES (@recipients,@body,@subject,GETDATE(),@IsNegative,@IsSend,@AgentCode)
	...
	BEGIN CATCH
		...
		INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES('[Corp].[st_SaveDeposit]', GETDATE(), @ErrorMessage)
	END CATCH
