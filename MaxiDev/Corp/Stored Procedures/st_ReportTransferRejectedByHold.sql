CREATE PROCEDURE [Corp].[st_ReportTransferRejectedByHold]
(
    @BeginDate DATETIME,
    @EndDate DATETIME,
    @IdStatus INT = NULL,
...
BEGIN
	...
	INSERT INTO @TransferRejected
		SELECT
			[IdTransfer]
			, [EnterByIdUser]
		FROM [dbo].[TransferHolds] WITH(NOLOCK)
	...
	INSERT INTO @TransferCloseRejected
		SELECT
			[IdTransferclosed]
			, [EnterByIdUser]
		FROM [dbo].[TransferclosedHolds] WITH(NOLOCK)
		WHERE [IdStatus] = @IdStatusKYC AND [IsReleased] = 0 AND [DateOfLastChange] >= @BeginDate AND [DateOfLastChange] < @EndDate
		UNION ALL
		SELECT
			[IdTransferclosed]
	...
	BEGIN
		...
		SELECT 
			...
			CU.[CurrencyName],
			CASE
				WHEN EXISTS (SELECT TOP 1 1 FROM [dbo].[TransferHolds] WITH(NOLOCK) WHERE [IdStatus] = @IdStatusKYC AND [IdTransfer] = T.[IdTransfer] AND [IsReleased] = 0) THEN ISNULL(S2.[StatusName],'')
				WHEN EXISTS (SELECT TOP 1 1 FROM [dbo].[TransferHolds] WITH(NOLOCK) WHERE [IdStatus] = @IdStatusDeny AND [IdTransfer] = T.[IdTransfer] AND [IsReleased] = 0) THEN ISNULL(S3.[StatusName],'')
				ELSE ''
			END [RejectedHold],
			U.[UserName],
			ISNULL(TN.[Note],''),
		...
		JOIN [dbo].[PaymentType] P WITH(NOLOCK) ON T.[IdPaymentType] = P.[IdPaymentType]
		JOIN [dbo].[Status] S1 WITH(NOLOCK) ON T.[IdStatus] = S1.[IdStatus]
		LEFT JOIN [dbo].[Status] S2 WITH(NOLOCK) ON @IdStatusKYC = S2.[IdStatus]
		LEFT JOIN [dbo].[Status] S3 WITH(NOLOCK) on @IdStatusDeny=s3.IdStatus
		JOIN [dbo].[Payer] PA WITH(NOLOCK) ON T.[IdPayer] = PA.[IdPayer]
		JOIN [dbo].[Gateway] G WITH(NOLOCK) ON T.[IdGateway] = G.[IdGateway]
		...
		JOIN [dbo].[Users] U WITH(NOLOCK) ON TR.[RejectedUserId] = U.[IdUser]
		JOIN (
			...
			JOIN [dbo].[TransferNote] TN WITH(NOLOCK) ON L.[IdTransferDetail] = TN.[IdTransferDetail]
			GROUP BY L.[IdTransfer], L.[IdTransferDetail]
			) M ON T.[IdTransfer] = M.[IdTransfer]
		LEFT JOIN [dbo].[TransferNote] TN WITH(NOLOCK) ON M.[IdTransferNote] = TN.[IdTransferNote]
		WHERE
		...
		SELECT 
			...
			T.[CurrencyName],
			CASE
				WHEN EXISTS (SELECT TOP 1 1 FROM [dbo].[TransferClosedHolds] WITH(NOLOCK) WHERE [IdStatus] = @IdStatusKYC AND [IdTransferClosed] = T.[IdTransferClosed] AND [IsReleased] = 0) THEN ISNULL(S2.[StatusName],'')
				WHEN EXISTS (SELECT TOP 1 1 FROM [dbo].[TransferClosedHolds] WITH(NOLOCK) WHERE [IdStatus] = @IdStatusDeny AND [IdTransferClosed] = T.[IdTransferClosed] AND [IsReleased] = 0) THEN ISNULL(S3.[StatusName],'')
				ELSE ''
			END [RejectedHold],
			U.[UserName],
			ISNULL(TN.[Note],''),
		...
		JOIN [dbo].[Users] U WITH(NOLOCK) ON TCR.[RejectedUserId] = U.[IdUser]
		JOIN (
			...
			FROM (
				...
				FROM [dbo].[TransferClosedDetail] TD WITH(NOLOCK)
				WHERE TD.[IdStatus] = 31
				GROUP BY TD.[IdTransferClosed] ) L
			JOIN [dbo].[TransferClosedNote] TN WITH(NOLOCK) ON L.[IdTransferClosedDetail] = TN.[IdTransferClosedDetail]
			GROUP BY L.[IdTransferClosed], L.[IdTransferClosedDetail]
			) M ON T.[IdTransferClosed] = M.[IdTransferClosed]
		LEFT JOIN [dbo].[TransferClosedNote] TN WITH(NOLOCK) ON M.[IdTransferClosedNote] = TN.[IdTransferClosedNote]
		WHERE
			T.[DateStatusChange] >= @BeginDate
			AND T.[DateStatusChange] < @EndDate
