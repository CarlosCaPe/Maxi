CREATE procedure [Corp].[st_UpdateTransferLookupOwb]
@IdTransfer int,
@Name nvarchar(max),
...
Begin try
	...
	begin
		...
		[Address], BornDate, City, Country, MoneySource, Relationship, Purpose, Zipcode, [State], SSNumber, Occupation, DateOfLastChange, IdGenericStatus, IdAgentCreatedBy, PhysicalIdCopy, EnterByIdUser)
		values
		(@Name, @FirstLastName, @SecondLastName, @ExpirationIdentification, @IdentificationNumber, @PhoneNumber, @CelullarNumber,
		@Address, @BornDate, @City, @Country, @MoneySource, @Relationship, @Purpose, @Zipcode, @State, @SSNumber, @Occupation, @DateOfLastChange, 1, @IdAgent, @PhysicalIdCopy, @EnterByIdUser);
	...
	begin
		...
		SSNumber = @SSNumber, Occupation = @Occupation, DateOfLastChange = @DateOfLastChange, PhysicalIdCopy = @PhysicalIdCopy, EnterByIdUser = @EnterByIdUser
...
Begin Catch
	...
	SET @HasError = 1;
	Set @Message = 'Error trying update transfer information';
	DECLARE @ErrorMessage varchar(max);
    Select @ErrorMessage=ERROR_MESSAGE();
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_UpdateTransferLookupOwb',Getdate(),@ErrorMessage);
End catch
