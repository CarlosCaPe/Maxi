CREATE PROCEDURE [Corp].[st_GetTransfersByHoldStatus]
@IdStatus int,
...
<log Date="31/10/2022" Author="cagarcia">MP-779 Se agregan columnas PayerCode, DateOnHold y Beneficiary</log>
<log Date="05/04/2023" Author="jfresendiz">BM-1204 Se agrega columna Marked para identificar los que necesitan validación extra del lado del front</log>
</ChangeLog>
********************************************************************/
...
Begin
       Select distinct top 1500 T.IdAgent,  A.AgentCode, A.AgentState, T.ClaimCode, T.CustomerName, T.CustomerFirstLastName, T.CustomerSecondLastName,
                     ...
                     --,Convert(bit,case when LC.IdUploadFile is not null or LT.IdUploadFile is not null then 1 else 0 end) HasFiles,                     
					  ...
					  ,T.FromStandByToKYC 
					  , (select count(*) from transferholds h where IsReleased is null and h.IdTransfer= T.IdTransfer and h.IdStatus=15) AS Marked 
	   into #tmpdata
       ...
       --ORDER BY T.DateOfTransfer desc
		...
		/*S26*/
		 select IdTransfer into #docs3              
					from TransferAutomaticNotification AS t WITH(NOLOCK)
		...
		select t.*, 
		 (
					 ...
					 ,'' AS 'Beneficiary'
					 ,getdate() AS 'DateOnHold' --No se usa del lado del front
					 ,'' AS 'PayerCode' 
		from #tmpdata T
...
Begin
	...
	-- End New RMM
	   ...
	   BEGIN
            ...
            0 releasedCount, t.IdGateway, gat.GatewayName
			, T.[AgentNotificationSent]
			,T.FromStandByToKYC, 0 Marked  into #tmpdata24
		   From [Transfer] T with(nolock)
		   inner join [Agent] A WITH(NOLOCK) ON T.IdAgent = A.IdAgent
		   inner join [Customer] C WITH(NOLOCK) ON T.IdCustomer = C.IdCustomer
		   inner join [Payer] P WITH(NOLOCK) ON T.IdPayer = P.IdPayer
	   ...
	   BEGIN
	    ...
	    BEGIN 
					SELECT TOP 1500 T.IdAgent,  A.AgentCode, T.ClaimCode, A.AgentState, T.CustomerName, T.CustomerFirstLastName, T.CustomerSecondLastName,
									...
									, T.[AgentNotificationSent]
									,T.FromStandByToKYC
									,isnull(T.BeneficiaryName, '') + ' ' + isnull(T.BeneficiaryFirstLastName, '') + ' ' + isnull(T.BeneficiarySecondLastName, '') AS 'Beneficiary'
									, DateOnHold = (SELECT TOP 1 TD.DateOfMovement FROM TransferDetail TD WITH(NOLOCK) WHERE TD.IdTransfer = T.IdTransfer AND TD.IdStatus = 29 ORDER BY TD.DateOfMovement DESC)
									, PayerCode = (SELECT TOP 1 TN.Note FROM TransferDetail TD WITH(NOLOCK) INNER JOIN TransferNote TN WITH(NOLOCK) ON TN.IdTransferDetail = TD.IdTransferDetail WHERE TD.IdTransfer = T.IdTransfer AND TD.IdStatus = 29 ORDER BY TD.DateOfMovement DESC)
									,0 Marked 
									INTO #tmpdataGl2
					FROM [Transfer] T WITH(NOLOCK) 
					INNER JOIN [Agent] A WITH(NOLOCK) ON T.IdAgent = A.IdAgent
