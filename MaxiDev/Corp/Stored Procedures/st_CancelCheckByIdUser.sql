CREATE PROCEDURE [Corp].[st_CancelCheckByIdUser]
(
    @IdCheck INT,
    @EnterByIdUser INT,
    @Note NVARCHAR(MAX),
...
set @HasError = 0
set @Message = ''
IF EXISTS (SELECT TOP 1 1 FROM [dbo].[Checks] WITH (NOLOCK) WHERE [IdCheck]=@IdCheck AND [IdStatus]=20)
begin
...
INSERT INTO [dbo].[CheckDetails]
	...
	IF NOT EXISTS (SELECT TOP 1 1 FROM [dbo].[Agent] WITH (NOLOCK) WHERE [IdAgentCommunication] IN (1,4) AND [IdAgent] = @AgentId)
		RETURN
...
Begin Catch
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_CancelCheck',Getdate(),@ErrorMessage)                                                                                            
End Catch
