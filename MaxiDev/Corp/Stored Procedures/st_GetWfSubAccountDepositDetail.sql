CREATE PROCEDURE [Corp].[st_GetWfSubAccountDepositDetail]
(
    @DepositData XML
)
AS
...
INSERT INTO #Deposit2  -- CR M00247
...
BEGIN    
    ...
    SET @TotalDeposits = ISNULL((SELECT COUNT(1) FROM dbo.AgentDeposit WITH(NOLOCK) WHERE IdAgent=@IdAgent AND Amount=@Amount AND (dbo.RemoveTimeFromDatetime([DepositDate])=@DepositDate) AND @BankName=BankName),0)
...
BEGIN CATCH
	...
	VALUES('[Corp].[st_GetWfSubAccountDepositDetail]',GETDATE(),@ErrorMessage)
END CATCH
