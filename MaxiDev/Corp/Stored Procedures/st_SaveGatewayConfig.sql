CREATE PROCEDURE [Corp].[st_SaveGatewayConfig]
(
	@IdStatus int, 
	@IdUser int,
	@Note nvarchar(max),
...
BEGIN TRY
	...
	select Mail, IdGateway, IdGenericStatus, IsInfoRequired, IdUser, DateOfLastChange, IdGatewayConfigMail from GatewayConfigMail with(nolock)
...
BEGIN CATCH
	 ...
	 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_SaveGatewayConfig', Getdate(), @ErrorMessage)    
END CATCH
