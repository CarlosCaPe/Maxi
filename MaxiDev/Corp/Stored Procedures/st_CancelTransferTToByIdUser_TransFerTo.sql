CREATE PROCEDURE [Corp].[st_CancelTransferTToByIdUser_TransFerTo]
(
    @IdLenguage int,
    @EnterByIdUser int,    
...
declare 
    ...
    select          
        ...
        @operator=operator
    from 
        TransferTo.[TransferTTo] with(nolock)
    where 
    ...
    select @login=username from users with(nolock) where iduser=@EnterByIdUser
    ...
    --Afectar Balance
         ...
         select @CountryCode=CountryCode from TransferTo.Country with(nolock) where countryname=@Country
         
         set @Country=upper(@Country)

         set @CountryCode= isnull(@CountryCode,@Country)

         EXEC	[Corp].[st_OtherProductToAgentBalance]
...
Begin Catch                                                                                        
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_CancelTransferTToByIdUser_TransFerTo',Getdate(),@ErrorMessage)                                                                                            
End Catch      
