CREATE PROCEDURE [Corp].[st_DashboardbyOtherProductByStateBP]
(    
    @WeeksAgo int,
...
Select SUM(NumTran) as TotalWeeksAgo, IdAgent into #T1
	from(
		select SUM(1) as NumTran, A.IdAgent from billpaymenttransactions T  
		...
		union all
		select SUM(1)*-1 as NumTran, A.IdAgent from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
...
Select SUM(NumTran) as TotalToday, sum(AmountInDollars) TotalAmountInDollarsToday, IdAgent into #T2
	from(
		select SUM(1) as NumTran,sum(corpcommission) AmountInDollars, A.IdAgent from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		...
		union all
		select SUM(1)*-1 as NumTran,(-1)*sum(corpcommission) AmountInDollars, A.IdAgent from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)        
