CREATE procedure [Corp].[st_ReportOtherChargesCPV2]
(
        @StartDate datetime,
		@EndDate datetime,
        @IdAgent int,
...
insert into #result2
            select 
                ...
                inner join 
	                AgentCollection AC WITH(NOLOCK) on AC.IdAgentCollection = ACD.IdAgentCollection 
                inner join
	            AgentCollectionConcept ACC WITH(NOLOCK) on ACC.IdAgentCollectionConcept = ACD.IdAgentCollectionConcept
                join agent a WITH(NOLOCK) on a.idagent=ac.idagent
                join users u WITH(NOLOCK) on u.iduser=ACD.enterbyiduser
