CREATE procedure [Corp].[st_GetTransferDetailByIdTransfer]
(
	@IdTransfer int, 
	@Start int = 1, 
	@PageSize int = 10,  
...
<log Date="14/04/2023" Author="jfresendiz"> BM-1206 Se añade el motivo de la cancelación cuando exista</log>
</ChangeLog>
*********************************************************************/
    set @Start = @Start-1
	...
	BEGIN
		...
		INNER JOIN [Transfer] T with(nolock) ON T.IdTransfer = A.IdTransfer
		...
		order by IsNull(B.EnterDate, A.DateOfMovement), A.IdStatus  desc; -- CR - M00249
	End 
	Else 
	Begin
		insert into @Detail (IdStatus, StatusName, DateDeatil, Users, Note, IdMessage, IdGenericStatus,idReasonForCancel,NoteTypeId)
	...
	if(@isAll = 0)
		BEGIN
			...
			DateDeatil, 
			Users, 
			Note=isnull(CASE WHEN Det.NoteTypeId = 2 THEN 'Reason: '+RC.ReasonEn+'' END,'') + CASE WHEN Note <> '' AND Det.NoteTypeId = 2 THEN ', Note: ' + Note ELSE Note END, 
			IdMessage, 
			det.IdGenericStatus
			FROM @Detail Det
			LEFT JOIN ReasonForCancel RC with(nolock) ON RC.IdReasonForCancel = det.idReasonForCancel
			WHERE Id BETWEEN @Start + 1 AND @Start + @PageSize;

			select Id, IdStatus, StatusName, DateDeatil,Users, Note, IdMessage, IdGenericStatus 
			from #Detail2 
			order by DateDeatil desc -- CR - M00249
		END
	ELSE
		BEGIN
			insert into #Detail2 -- CR - M00249
