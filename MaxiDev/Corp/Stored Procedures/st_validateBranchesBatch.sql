CREATE PROCEDURE [Corp].[st_validateBranchesBatch] (@idPayer INT , @data xml)
/********************************************************************
...
AS BEGIN try
   ...
   IF (@data IS NOT NULL ) BEGIN
	...
	EXECUTE [Corp].[st_findLocationByName] @name,0,@idOut OUT
	...
	SELECT code FROM [dbo].Branch WITH (NOLOCK) WHERE IdPayer = @idPayer
	)
...
END try
BEGIN CATCH
	INSERT INTO ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage) values ('st_validateBranchesBatch - ' + convert(varchar(max),ERROR_LINE()),GETDATE(),ERROR_MESSAGE())
END CATCH
