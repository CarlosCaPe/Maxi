CREATE procedure [Corp].[st_FilterByStatusForExcel]                    
(                    
...
*********************************************************************/
	...
	SET @SQL = 'Select B.IdAgent, A.IdTransfer, A.IdStatus, A.ClaimCode, A.DateOfTransfer, B.AgentCode, 
		...
		From Transfer A  WITH(NOLOCK)              
			...
			Join TransferDetail H WITH(NOLOCK) on (A.IdTransfer=H.IdTransfer)
			Join Gateway I WITH(NOLOCK) on (I.IdGateway=A.IdGateway)
			Join Currency J WITH(NOLOCK) on (J.IdCurrency=F.IdCurrency)
		WHERE '
		...
		SET @SQL2='                    
			...
			From TransferClosed A   WITH(NOLOCK)             
				Join Agent B WITH(NOLOCK) on (A.IdAgent=B.IdAgent)                
				Join TransferClosedDetail C WITH(NOLOCK) on (C.IdTransferClosed=A.IdTransferClosed)            
			Where '
...
Begin                              
 Set @SQL=@SQL+'A.IdCustomer in (Select IdCustomer from CardVIP where IdGenericStatus=1  and CardNumber='+char(39)+@CardNumber+char(39)+')'+' And '  
 Set @SQLCount=@SQLCount+'A.IdCustomer in (Select IdCustomer from CardVIP where IdGenericStatus=1  and CardNumber='+char(39)+@CardNumber+char(39)+')'+' And '                  
                               
 Set @SQL2=@SQL2+'A.IdCustomer in (Select IdCustomer from CardVIP where IdGenericStatus=1  and CardNumber='+char(39)+@CardNumber+char(39)+')'+' And '                  
 Set @SQLCount2=@SQLCount2+'A.IdCustomer in (Select IdCustomer from CardVIP where IdGenericStatus=1  and CardNumber='+char(39)+@CardNumber+char(39)+')'+' And '                             
End    
...
Begin Catch   
 ...
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_FilterByStatus',Getdate(),@ErrorMessage)                                                
End Catch 
