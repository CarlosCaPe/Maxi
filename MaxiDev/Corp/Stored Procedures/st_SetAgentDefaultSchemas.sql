CREATE PROCEDURE [Corp].[st_SetAgentDefaultSchemas]
(
     @IdAgent int,
     @EnterByIdUser int,     
     @IdLenguage int,
...
if exists (select top 1 1 from agentschema where idagent=@IdAgent)
begin
    Set @HasError=1	
...
while exists (select top 1 1 from #TempAgentSchema)
begin    
    ...
    insert into AgentSchemaDetail
		(IdAgentSchema,IdPayerConfig,SpreadValue,DateOfLastChange,EnterByIdUser,IdFee, IdCommission)
			...
			from agentschemadetail where idagentschema=@IdAgentSchema
...
Begin Catch
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_SetAgentDefaultSchemas',Getdate(),@ErrorMessage)
End Catch
