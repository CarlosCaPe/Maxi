CREATE procedure [Corp].[st_SearchBillPayments_Operation]
(              
	@FromDate DATETIME,
	@ToDate DATETIME,
		...
		begin
			INSERT INTO #BillPaymentsProducts
				...
				WHERE
					...
					AND [TR].[Name] like '%' + ISNULL(@Biller,[TR].[Name]) + '%' --BM-581
	...
	BEGIN
   		...
   		INSERT INTO #BillPaymentsProducts
			SELECT
				...
				, [A].[AgentCode]
				, [ProviderId] = 19
				, [ProviderName] = (select Description from OtherProducts WITH (NOLOCK) where IdOtherProducts= 19)
