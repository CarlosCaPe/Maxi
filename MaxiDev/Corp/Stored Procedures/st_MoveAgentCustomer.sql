CREATE  PROCEDURE [Corp].[st_MoveAgentCustomer]
(
    @IdAgentOrigin int,
    @IdAgentDestiny int,
    @EnterByIdUser int,
...
Begin Try      
 ...
 if not exists (select top 1 1 from customer c  with (nolock) where IdAgentCreatedBy=@IdAgentOrigin)
 begin
    Set @HasError = 1                                       
	...
	SELECT idElasticCustomer FROM Customer with (nolock) WHERE IdAgentCreatedBy = @IdAgentOrigin
...
Begin Catch                                                
 ...
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('[Corp].[st_MoveAgentCustomer]',Getdate(),@ErrorMessage)                                                
End Catch 
