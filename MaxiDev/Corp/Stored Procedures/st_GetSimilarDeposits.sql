CREATE PROCEDURE [Corp].[st_GetSimilarDeposits]
(
    @IdAgent INT,
    @Amount money, 
...
FROM 
    dbo.AgentDeposit d with(nolock)
JOIN 
    AgentCollectType c with(nolock) ON d.IdAgentCollectType =c.IdAgentCollectType
JOIN
    dbo.Users u with(nolock) ON d.EnterByIdUser=u.IdUser
WHERE 
    IdAgent=@IdAgent AND
    Amount=@Amount AND
