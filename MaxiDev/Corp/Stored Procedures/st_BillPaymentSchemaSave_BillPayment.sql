CREATE PROCEDURE [Corp].[st_BillPaymentSchemaSave_BillPayment]
   @IdAgent int
   , @IdAggregator int
	 , @IdFee int 
	    ,@IdBiller int
	 ...
	 if exists(	
		 ...
		 begin 
				 ...
				 set @IdPreviousFee = (Select B.IdFee 
										from BillPayment.AgentForBillers B WITH (NOLOCK)
										inner join BillPayment.Billers Bi WITH (NOLOCK) on Bi.IdBiller = B.IdBiller and Bi.IdAggregator = @IdAggregator 
		...
		begin
			...
			--print '1'
			set @IdPreviousFee = (Select B.IdFee from BillPayment.StateForBillers B WITH (NOLOCK) inner join BillPayment.Billers Bi WITH (NOLOCK) on Bi.IdBiller = B.IdBiller and Bi.IdAggregator = @IdAggregator where B.IdBiller = @IdBiller and B.IdState = @IdState )
			set @IdPreviousCommission = (Select B.IdCommission from BillPayment.StateForBillers B WITH (NOLOCK) inner join BillPayment.Billers Bi WITH (NOLOCK) on Bi.IdBiller = B.IdBiller and Bi.IdAggregator = @IdAggregator where B.IdBiller = @IdBiller and B.IdState = @IdState )
		...
		BEGIN
		--print '2'
			INSERT [dbo].AgentSchemaDetailFeeLog (IdAgentSchema, IdPayerConfig, IdPreviousFee, IdCurrentFee, DateOfLastChange, EnterByIdUser, IdAgent)
		...
		BEGIN
		--print '3'
			INSERT [dbo].AgentSchemaDetailCommissionLog (IdAgentSchema, IdPayerConfig, IdPreviousCommission, IdCurrentCommission, DateOfLastChange, EnterByIdUser, IdAgent)
...
Begin Catch 
 set @HasError = 1
 set @Message='Error'
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_BillPaymentSchemaSave_BillPayment',Getdate(),ERROR_MESSAGE())    
End Catch
