CREATE PROCEDURE [Corp].[st_RegaliiToAgentBalance]        
(        
    @IdTransaction int,
    @IdOtherProduct int,
    @IdAgent int,
...
If not Exists (Select 1 from AgentCurrentBalance where IdAgent=@IdAgent) 
begin          
...
begin catch
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_OtherProductToAgentBalance',Getdate(),@ErrorMessage)                                                                                            
end catch
