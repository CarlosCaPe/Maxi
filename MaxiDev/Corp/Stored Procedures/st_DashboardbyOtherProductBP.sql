CREATE PROCEDURE [Corp].[st_DashboardbyOtherProductBP]
(    
    @WeeksAgo int,
...
set @OtherProductPrefix='BP'
set @IdOtherProduct=1
...
Select SUM(NumTran) as TotalWeeksAgo, AgentState into #T1
	from(
		select SUM(1) as NumTran, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.Paymentdate>= @StartDate and T.Paymentdate<@EndDate and t.status in (select id from @TransactionStatusForPositive)
		...
		select SUM(1)*-1 as NumTran, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.CancelDate>= @StartDate and T.CancelDate<@EndDate and t.status in (select id from @TransactionStatusForNegative)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState
	) LT
...
Select SUM(NumTran) as TotalToday,sum(AmountInDollars) TotalAmountInDollarsToday, AgentState into #T2
	from(
		select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.Paymentdate>= @NowStart and T.Paymentdate<@Now and t.status in (select id from @TransactionStatusForPositive)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDate>= @NowStart and T.CancelDate<@Now and t.status in (select id from @TransactionStatusForNegative)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
	) LT
...
Select SUM(NumTran) as TotalOneMonthAgo,sum(AmountInDollars) TotalAmountInDollarsOneMonthAgo, AgentState into #T3 
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.Paymentdate>= @OneMonthAgoSD and T.Paymentdate<@OneMonthAgoED and t.status in (select id from @TransactionStatusForPositive)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDate>= @OneMonthAgoSD and T.CancelDate<@OneMonthAgoED and t.status in (select id from @TransactionStatusForNegative)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
	) LT
...
Select SUM(NumTran) as TotalTwoMonthAgo,sum(AmountInDollars) TotalAmountInDollarsTwoMonthAgo, AgentState into #T4 
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.Paymentdate>= @TwoMonthAgoSD and T.Paymentdate<@TwoMonthAgoED and t.status in (select id from @TransactionStatusForPositive)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDate>= @TwoMonthAgoSD and T.CancelDate<@TwoMonthAgoED and t.status in (select id from @TransactionStatusForNegative)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
	) LT
...
Select SUM(NumTran) as TotalThreeMonthAgo,sum(AmountInDollars) TotalAmountInDollarsThreeMonthAgo, AgentState into #T5 
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.Paymentdate>= @ThreeMonthAgoSD and T.Paymentdate<@ThreeMonthAgoED and t.status in (select id from @TransactionStatusForPositive)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDate>= @ThreeMonthAgoSD and T.CancelDate<@ThreeMonthAgoED and t.status in (select id from @TransactionStatusForNegative)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
	) LT
...
Select SUM(NumTran) as TotalCurrentMonth,sum(AmountInDollars) TotalAmountInDollarsCurrentMonth, AgentState into #T6
	from(
        select SUM(1) as NumTran,sum(CorpCommission) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.Paymentdate>= @CurrentMonthSD and T.Paymentdate<@CurrentMonthED and t.status in (select id from @TransactionStatusForPositive)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState		
		...
		select SUM(1)*(-1) as NumTran,sum(CorpCommission)*(-1) AmountInDollars, A.AgentState from billpaymenttransactions T
		Join Agent A on (T.IdAgent=A.IdAgent)        
		where T.CancelDate>= @CurrentMonthSD and T.CancelDate<@CurrentMonthED and t.status in (select id from @TransactionStatusForNegative)
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))				
                and a.idagentstatus in (select id from @tStatus)
		group by A.AgentState					
	) LT
