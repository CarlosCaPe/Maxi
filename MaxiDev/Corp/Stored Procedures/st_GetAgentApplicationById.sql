CREATE PROCEDURE [Corp].[st_GetAgentApplicationById]
(
...
select 
	...
	,isnull(OSID.StateName, '') AS 'OwnerIDStateEmission'
	,isnull(OCID.CountryName, '') AS 'OwnerIDCountryEmission'
	,isnull(OSID.IdState, 0) AS 'OwnerIdStateEmission_IdState'
	,isnull(OCID.IdCountry, 0) AS 'OwnerIDCountryEmission_IdCountry'
	,a.[TypeTaxID]
from AgentApplications a WITH(NOLOCK) 
    ...
    LEFT JOIN State OSID WITH(NOLOCK) ON OSID.IdState = o.IdStateEmission
    LEFT JOIN Country OCID WITH(NOLOCK) ON OCID.IdCountry = o.IdCountryEmission
...
/*Fees*/    
SELECT F.IdAgentApplication, F.PercentageFee AS 'AgentPercentageFee', (100 - F.PercentageFee) AS 'CorpPercentageFee', C.CountryName
FROM dbo.FeesAgentApplication F INNER JOIN
	dbo.Country C ON C.IdCountry = F.IdCountry
WHERE F.IdAgentApplication = @idAgentApplication;


/*Exceptions*/
SELECT E.ExceptionAgentFee AS 'AgentExceptionFee', (100 - E.ExceptionAgentFee) AS 'CorpExceptionFee', C.CountryName, P.PayerName
FROM dbo.ExceptionsAgentApplication E INNER JOIN
	dbo.Country C ON c.IdCountry = E.IdCountry INNER JOIN
	dbo.Payer P ON P.IdPayer = E.IdPayer
