CREATE PROCEDURE [Corp].[st_TransferDetail]
( 
    @IdTransfer Int, 
...
<log Date="19/07/2022" Author="cagarcia">SD1-1863 Add CustomerReceiveSMS fields</log>
<log Date="25/10/2022" Author="cagarcia">MP-1269/SD-2377 Se agrega campo para indicar que se puede reenviar SMS para envios domesticos</log>
<log Date="14/03/2023" Author="jfresendiz">BM-665 - Se retorna la dirección de la agencia</log>
<log Date="2023/02/22" Author="jfresendiz">BM-1045 Se retornan los campos TaxID</log>
</ChangeLog>
********************************************************************/
SET NOCOUNT ON

DECLARE @CanResendSMS BIT

IF EXISTS (SELECT TOP 1 1 FROM dbo.TransferDetail TD WITH(NOLOCK) INNER JOIN 
							dbo.Transfer T WITH(NOLOCK) ON T.IdTransfer = TD.IdTransfer  
			WHERE T.IdTransfer = @IdTransfer AND Td.IdStatus = 23 AND T.IdPaymentType = 6)
BEGIN
	SET @CanResendSMS = 1
END
ELSE
...
BEGIN
	...
	SELECT
		...
		, T.OperationFee
		, T.DateOfTransferUTC
		, NULL	IdDialingCodeCustomerPhoneNumber
		, NULL	DialingCode
		, 0		IsValidCustomerPhoneNumber
		, pt.IdPosTransfer		PosTransfer_IdPosTransfer
		, pt.AuthorizationNo	PosTransfer_AuthorizationNo
		, pt.BatchNo			PosTransfer_BatchNo
		, pt.ReferenceNo		PosTransfer_ReferenceNo
		, pt.TerminalId			PosTransfer_TerminalId
		, pt.MerchantId			PosTransfer_MerchantId
		, pt.AccountNo			PosTransfer_AccountNo
		, pt.IdPosStatus	PosTransfer_IdPosStatus
		, pt.IdCardEntryMode	PosTransfer_IdCardEntryMode
		, pt.IdCardType	PosTransfer_IdCardType
		, pt.IdPosActionType	PosTransfer_IdPosActionType
		, ps.PosStatus			PosTransfer_PosStatus
		, cem.CardEntryMode		PosTransfer_CardEntryMode
		, ct.CardType			PosTransfer_CardType
		, pat.PosActionType		PosTransfer_PosActionType
		, N.ReceiveSMS			CustomerReceiveSMS
	...
	FROM [dbo].[Transfer] T WITH (NOLOCK)
		...
		JOIN PaymentMethod pm WITH(NOLOCK) ON pm.IdPaymentMethod = T.IdPaymentMethod
		LEFT JOIN PosTransfer pt WITH(NOLOCK) ON pt.IdTransfer = T.IdTransfer
		LEFT JOIN PosStatus ps WITH(NOLOCK) ON ps.IdPosStatus = pt.IdPosStatus
		LEFT JOIN CardEntryMode cem WITH(NOLOCK) ON cem.IdCardEntryMode = pt.IdCardEntryMode
		LEFT JOIN CardType ct WITH(NOLOCK) ON ct.IdCardType = pt.IdCardType
		LEFT JOIN PosActionType pat WITH(NOLOCK) ON pat.IdPosActionType = pt.IdPosActionType
		LEFT JOIN PreTransfer prt WITH(nolock) ON prt.IdTransfer = T.IdTransfer
...
---------------------- From TransactionClosed -------------------------------------------------------- 
	...
	From [dbo].[TransferClosed] T WITH (NOLOCK)
		...
		JOIN PaymentMethod pm WITH(NOLOCK) ON pm.IdPaymentMethod = T.IdPaymentMethod
		LEFT JOIN PosTransfer pt WITH(NOLOCK) ON pt.IdTransferClosed = T.IdTransferClosed
		LEFT JOIN PosStatus ps WITH(NOLOCK) ON ps.IdPosStatus = pt.IdPosStatus
		LEFT JOIN CardEntryMode cem WITH(NOLOCK) ON cem.IdCardEntryMode = pt.IdCardEntryMode
		LEFT JOIN CardType ct WITH(NOLOCK) ON ct.IdCardType = pt.IdCardType
		LEFT JOIN PosActionType pat WITH(NOLOCK) ON pat.IdPosActionType = pt.IdPosActionType
