CREATE procedure [Corp].[st_InsertStateByBiller_BillPayment]
...
begin try 
 ...
 if exists(
           select  
            1
           from 
            BillPayment.StateForBillers St WITH (NOLOCK)
           where 
...
set  @idFeeDefualt= 
  ...
  set  @idFee = isnull((select idfee from  BillPayment.StateForBillers with (nolock)  where idbiller= @IdBiller  and IdStateBiller= (select max(IdStateBiller) from BillPayment.StateForBillers with (nolock) where idbiller=@IdBiller )),@idFeeDefualt)       
  
  set  @IdCommission =  isnull((select IdCommission from  BillPayment.StateForBillers  with (nolock)  where idbiller= @IdBiller  and IdStateBiller= (select max(IdStateBiller) from BillPayment.StateForBillers with (nolock) where idbiller=@IdBiller )) ,@IdCommissionDefualt) 
...
Begin Catch 
 set @HasError = 1
 set @Message='Error'
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Corp.st_InsertStateByBiller_BillPayment',Getdate(),ERROR_MESSAGE())    
End Catch
