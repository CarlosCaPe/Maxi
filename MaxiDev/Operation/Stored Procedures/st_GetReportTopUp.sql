CREATE PROCEDURE [Operation].[st_GetReportTopUp]
(
...
*********************************************************************/
 ...
 --   DECLARE @Message nvarchar(max)
	...
	SET @Tot = 0
	SELECT @Tot = count(1)
	FROM operation.ProductTransfer t 
		join agent a on t.idagent = a.idagent
		join dbo.[OtherProductStatus] s on t.[IdStatus] = s.[IdStatus]
	...
	BEGIN 
		...
		INSERT INTO #Result 
			...
			FROM operation.ProductTransfer t
				JOIN TransferTo.[TransferTTo] tt on t.IdProductTransfer=tt.IdProductTransfer and dbo.fnDeleteFormatPhoneNumber(tt.[Msisdn]) like '%'+isnull(@CustomerNumber,'')+'%' and dbo.fnDeleteFormatPhoneNumber(tt.Destination_Msisdn) like '%'+isnull(@CellularNumber,'')+'%' 
				JOIN agent a on t.idagent = a.idagent
				JOIN dbo.status s ON t.[IdStatus] = s.[IdStatus]
				JOIN providers pr ON pr.idprovider = t.idprovider 
				LEFT JOIN users u ON t.EnterByIdUser = u.IdUser
			WHERE 
		...
		INSERT INTO #Result
			SELECT 
				...
				AgentState AgentState
			FROM operation.ProductTransfer t
				join pureminutestopuptransaction tt on t.IdProductTransfer=tt.IdProductTransfer
				join agent a on t.idagent=a.idagent
				join dbo.status s on t.[IdStatus]=s.[IdStatus]
				join CarrierPureMinutesTopUp cp on tt.CarrierID=cp.IdCarrierPureMinutesTopUp
				join CountryPureMinutesTopUp cup on tt.CountryID=cup.IdCountryPureMinutesTopUp
				join providers pr on pr.idprovider=t.idprovider
				left join users u on t.EnterByIdUser=u.IdUser
			WHERE 
				T.DateOfCreation >= CASE WHEN @IdTransfer IS NULL THEN ISNULL(@BeginDate, T.DateOfCreation) ELSE T.DateOfCreation END
		...
		INSERT INTO #Result
			SELECT 
				...
				AgentState AgentState
			FROM operation.ProductTransfer t
				join lunex.TransferLN tt on t.IdProductTransfer=tt.IdProductTransfer and dbo.fnDeleteFormatPhoneNumber(tt.phone) like '%'+isnull(@CustomerNumber,'')+'%' and dbo.fnDeleteFormatPhoneNumber(tt.TopupPhone) like '%'+isnull(@CellularNumber,'')+'%'
				join agent a on t.idagent=a.idagent
				join dbo.status s on t.[IdStatus]=s.[IdStatus]
				join lunex.Product p on p.SKU=tt.SKU
				join Operation.Country cup on p.IdCountry=cup.IdCountry
				join Operation.Carrier cp on p.IdCarrier=cp.IdCarrier
				join providers pr on pr.idprovider=t.idprovider
				left join users u on t.EnterByIdUser=u.IdUser
			WHERE 
				T.DateOfCreation >= CASE WHEN @IdTransfer IS NULL THEN ISNULL(@BeginDate, T.DateOfCreation) ELSE T.DateOfCreation END
