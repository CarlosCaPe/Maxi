...
CREATE PROCEDURE [BillPayment].[st_GetSchemaBillersByIdAgent]
(
    @idagent int,   
    @idAggregator int, 
    @IdLenguage int,    
...
--Insert Into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage) values ('BillPayment.st_GetSchemaBillersByIdAgent', GETDATE(),'@idagent: ' + CONVERT(varchar(MAX),@idagent)+ ' @idAggregator: ' + CONVERT(varchar(MAX),@idAggregator)+ ' @IdLenguage: ' + CONVERT(varchar(MAX),@IdLenguage)+ ' @idUser: ' + CONVERT(varchar(MAX),@idUser)) 
    ...
    Select 
		...
		A.DateForCommision,
		B.IdStatus
	from BillPayment.AgentForBillers A
	    inner join BillPayment.Billers B on B.IdBiller = A.IdBiller
	    inner join FeeByOtherProducts F on F.IdFeeByOtherProducts = A.IdFee
	    inner join CommissionByOtherProducts C on C.IdCommissionByOtherProducts = A.IdCommission
	...
	Select 
		...
		B.IdStatus		 
	from BillPayment.StateForBillers S
	    inner join BillPayment.Billers B on B.IdBiller = S.IdBiller
	    inner join FeeByOtherProducts F on F.IdFeeByOtherProducts = S.IdFee
	    inner join CommissionByOtherProducts C on C.IdCommissionByOtherProducts = S.IdCommission
	where B.IdAggregator = @idAggregator
	and S.IdState = @idState
	and S.IdBiller not in (Select C.IdBiller from BillPayment.AgentForBillers C where C.IdAgent = @idagent)
	and S.IDStatus=1
	order by IdBiller;
...
Begin Catch 
    ...
    Insert into Soporte.InfoLogForStoreProcedure(StoreProcedure,InfoDate,InfoMessage)
	   Values('st_GetSchemaBillersByIdAgent'
			,Getdate()
