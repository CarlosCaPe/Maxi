...
CREATE PROCEDURE [BillPayment].[st_UpdateAggregatorStatus]
   @IdAggregator int
   , @IdStatus int
	 , @IdUser int 
	 , @HasError int out
...
Begin Catch 
 set @HasError = 1
 set @Message='Error'
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_UpdateAggregatorStatus',Getdate(),ERROR_MESSAGE())    
End Catch
