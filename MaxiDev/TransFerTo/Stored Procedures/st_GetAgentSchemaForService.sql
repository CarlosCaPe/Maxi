...
CREATE procedure [TransFerTo].[st_GetAgentSchemaForService]
(
    @Agentcode nvarchar(max),
    @CountryName nvarchar(max)
)
as
declare @IdAgent int, @IdCountry int

select @IdAgent=idagent from agent where agentcode=@Agentcode
select @IdCountry=idcountry from [TransFerTo].country where countryname=@CountryName
...
where s.idcountry=isnull(@IdCountry,s.idcountry) and isdefault=0 and s.IdGenericStatus=1 and IdSchema in (select IdSchema from  [TransFerTo].[AgentSchema] where idagent=@IdAgent)
union all
select @IdAgent IdAgent,IdSchema,SchemaName,s.IdCountry,countryname,s.IdCarrier,carriername,s.IdProduct,p.Product,p.RetailPrice,BeginValue,EndValue,Commission,IsDefault ,
case 
    when s.idcountry is null and s.idcarrier is null and s.IdProduct is null then 0
