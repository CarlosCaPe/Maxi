...
CREATE PROCEDURE [Checks].[st_CheckUpdateVerifyHold]
 (
    @EnterByIdUser INT, 
...
********************************************************************/
	 ...
	 BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					EXEC checks.st_SaveChangesToCheckLog @IdCheck, @HoldAcceptedStatus, @Note, @EnterByIdUser
				...
				BEGIN
					...
					EXEC checks.st_SaveChangesToCheckLog @IdCheck, 31, @Note, @EnterByIdUser
					EXEC [dbo].st_DismissComplianceNotificationByIdCheck @idCheck, @IsSpanishLanguage,  @HasError OUTPUT,  @Message OUTPUT
		...
		BEGIN 
			...
			BEGIN
				EXEC [msg].[st_SENDKYCCheckMessage]
					@IdCheck = @IdCheck, 
					@MessageTEXT = @Note, 
	...
	BEGIN CATCH
		...
		INSERT  INTO ErrorLogForStoreProcedure (StoreProcedure, ErrorDate, ErrorMessage)Values('st_CheckUpdateVerifyHold', GETDATE(), @ErrorMessage)
	END CATCH
