CREATE procedure [Regalii].[st_SaveCurrenciesSpread]
(
    @IdCurrenciesSpread int out,
    @IdCurrency int,
    @IdAgent int = null,
    @Spread money,
    @IdGenericStatus int ,
...
begin
    if exists (select top 1 1 from Regalii.[CurrenciesSpread] where IdCurrency=@IdCurrency and isnull(Idagent,0)=isnull(@IdAgent,0))
    begin
        Set @MessageError=1          
    ...
    set @IdCurrenciesSpread = SCOPE_IDENTITY()
...
begin
    ...
    set @dataxml = (select *  from Regalii.[CurrenciesSpread] where IdCurrenciesSpread=@IdCurrenciesSpread FOR XML RAW)
