...
<app> WebApi </app>
<Description> Sp para obtener todas las transferencias por fecha </Description>
...
CREATE PROCEDURE [MaxiMobile].[st_GetTransfersByAgentDate]
(
	@IdAgent INT,
	@Date datetime,
	@DateTo datetime
...
Select @DateFrom=dbo.RemoveTimeFromDatetime(@Date)
...
(
	idtransfer int
		...
		WHERE T.IdAgent = @IdAgent AND T.IdStatus = 41 and DateOfTransfer>=@DateFrom and DateOfTransfer<=@DateTo
		...
		select t.IdTransfer FROM [TRANSFER] T (nolock)                                					
		join maximobile.TransferAdditionalInfo i (nolock) on t.IdTransfer=i.IdTransfer and i.numdocs>0
		WHERE T.IdAgent = @IdAgent AND T.IdStatus = 29 and DateOfTransfer>=@DateFrom and DateOfTransfer<=@DateTo
		...
		FROM [TRANSFER] T    (NOLOCK)                         		
			where DateOfTransfer>=@DateFrom and DateOfTransfer <@DateTo 
			AND IdAgent=@IdAgent 
			AND t.IdTransfer not in (select IdTransfer from @fasttransfer)
...
SELECT 
		...
		FROM [TransferClosed] T  (NOLOCK)     
			where DateOfTransfer>=@DateFrom and DateOfTransfer <@DateTo and IdAgent=@IdAgent		
END TRY
BEGIN CATCH
