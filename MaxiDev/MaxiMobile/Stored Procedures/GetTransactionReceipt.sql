...
CREATE procedure [MaxiMobile].[GetTransactionReceipt]--9984553,18,11
(	@IdTransfer int,
	@IdCountryOrigin int,
...
IF EXISTS(SELECT 1 FROM TransferModify tm WHERE tm.NewIdTransfer = @IdTransfer)
BEGIN
	SET @IsChangeRequest = 1

	IF EXISTS(SELECT 1 FROM TransferModify tm JOIN TransferDetail td ON td.IdTransfer = tm.OldIdTransfer WHERE tm.NewIdTransfer = @IdTransfer AND td.IdStatus IN (21, 23))
