CREATE PROCEDURE [MaxiMobile].[st_EvaluateKYCRule]
(
	@IdTransfer int
)
AS
...
Begin try
	...
	declare @AmountInDollars money
	declare @AmountInMN money
	declare @IdCountryCurrency int

	declare @IdBranch int = NULL
	declare @IdCity int = NULL
	declare @Fee MONEY = NULL
	declare @ExRate MONEY = NULL
	declare @IdStateDestination int = null

	select @CustomerName = CustomerName, @CustomerFirstLastName = CustomerFirstLastName, @CustomerSecondLastName = CustomerSecondLastName, @BeneficiaryName = BeneficiaryName, 
		@BeneficiaryFirstLastName = BeneficiaryFirstLastName, @BeneficiarySecondLastName = BeneficiarySecondLastName, @IdPayer = IdPayer, @IdPaymenttype = IdPaymentType, 
		@IdAgent = IdAgent, @IdCountry = CustomerCountry, @IdGateway = IdGateway, @IdCustomer = IdCustomer, @IdBeneficiary = IdBeneficiary, @AmountInDollars = AmountInDollars, 
		@AmountInMN = AmountInMN, @IdCountryCurrency = IdCountryCurrency from transfer where IdTransfer = @IdTransfer
...
declare @MaxTimeDate datetime 
		...
		Where customerid=@IdCustomer and status=1 and 
...
Select IdRule,RuleName,Action,MessageInSpanish,MessageInEnglish,IsDenyList,SSNRequired,OccupationRequired,IsBlackList, ComplianceFormatId, ComplianceFormatName
	/*>> S35*/
	,IdTypeRequired
	,IdNumberRequired
	,IdExpirationDateRequired
...
DECLARE @ErrorMessage varchar(max)                                                                 
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_EvaluateKYCRule',Getdate(),@ErrorMessage)
End catch
