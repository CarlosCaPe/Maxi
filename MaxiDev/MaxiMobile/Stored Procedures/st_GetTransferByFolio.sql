...
<app> WebApi </app>
<Description> Sp que obtiene el detalle de una transferencia por folio y agencia </Description>
...
CREATE PROCEDURE [MaxiMobile].[st_GetTransferByFolio]
(
	@Folio INT,
	@IdAgent INT
)
...
(
	idtransfer int
		...
		WHERE T.IdAgent = @IdAgent AND T.IdStatus = 41 and folio=@Folio
		...
		select t.IdTransfer FROM [TRANSFER] T (nolock)                                					
		join maximobile.TransferAdditionalInfo i (nolock) on t.IdTransfer=i.IdTransfer and i.numdocs>0
		WHERE T.IdAgent = @IdAgent AND T.IdStatus = 29 and folio=@Folio

		--select * from @fasttransfer
		...
		FROM [TRANSFER] T    (NOLOCK)                         		
			where folio=@Folio and IdAgent=@IdAgent and t.IdTransfer not in (select IdTransfer from @fasttransfer)
...
SELECT 
		...
		FROM [TransferClosed] T  (NOLOCK)     
			where folio=@Folio and IdAgent=@IdAgent	
...
BEGIN CATCH
	...
	VALUES('[MaxiMobile].[st_GetTransferByFolio]',GETDATE(),@ErrorMessage)
END CATCH
