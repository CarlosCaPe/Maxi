CREATE procedure [MaxiMobile].[st_getAgentFee]
	@IdAgent int
as
/********************************************************************
<Author>rgaona</Author>
<app>MaxiAgente</app>
<Description>Tarifas para MaxiAgentMovil</Description>

<ChangeLog>
<log Date="30/04/2019" Author="rgaona">tarifas</log>
</ChangeLog>
*********************************************************************/
create table #salida
(
...
insert into #salida
	Select    
	 co.IdCountry
	  ...
	  , tot =count(1) 
	  , idrow= ROW_NUMBER() over (PARTITION BY b.IdAgentSchema ORDER BY b.IdAgentSchema, f.IdPaymentType) 
	  , orderS =
	       case 
	...
	LEFT JOIN 
		MaxiMobile.CountryFlag FL (NOLOCK)
		ON FL.IdCountry = CO.IdCountry	 
	where 
	 b.IdAgent=@IdAgent 
	...
	group by  
	 ...
	 , CO.CountryCode
	 having count(1)>1
	order by orderS, f.IdPaymentType



insert into #salida
Select     CO.IdCountry, FL.CountryFlag,co.CountryName,
...
INNER JOIN CountryCurrency CC (NOLOCK) ON b.IdCountryCurrency = CC.IdCountryCurrency 
...
group by   CO.IdCountry, FL.CountryFlag, co.CountryName, b.IdAgentSchema,b.SchemaName,f.IdPaymentType,e.IdFee, g.PayerName, CO.CountryCode,case when CountryCode='MEX' then 1 else 2 end
having count(1)=1
