CREATE PROCEDURE [Soporte].[sp_FixAgentBalanceSkips]
...
--{--#1
	...
	begin
		...
		set @Balance=(select Balance from #TmpAgBalVal where id=@Num1)
			...
			BEGIN
				SELECT 'Balance es diferente'
				set @Num1=@Max
				IF NOT EXISTS (SELECT * FROM Soporte.AgentToCorrect WHERE IdAgent = @IdAgent AND [BEGIN] = convert(DATE, @BeginDate))
				BEGIN
					SELECT 'Va a insertar agente a corregir' 
	...
	SELECT * FROM Soporte.AgentToCorrect
	

	if exists(select 1 from Soporte.AgentToCorrect)
	BEGIN
		...
		while exists (select 1 from Soporte.AgentToCorrect)
		begin
	
			select top 1 @idag=IdAgent,@Beg=[Begin] from Soporte.AgentToCorrect
	...
	BEGIN
		...
		BEGIN
			...
			@recipients='soportemaxi@boz.mx;bozservices@boz.mx;josesoto@boz.mx;',
			@subject=@Subject,
			@body=@XmlFormat,
			@body_format = 'HTML'
		END
...
DECLARE @ErrorMessage varchar(max)                                                                 
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('Soporte.sp_FixAgentBalanceSkips',Getdate(),@ErrorMessage)
End catch
