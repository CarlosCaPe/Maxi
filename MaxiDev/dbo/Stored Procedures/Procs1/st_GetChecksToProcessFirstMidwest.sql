CREATE PROCEDURE [dbo].[st_GetChecksToProcessFirstMidwest]
AS
/********************************************************************
<Author>Miguel Hinojo</Author>
<app>Maxi Host Manager Service</app>
<Description>Get checks that are on standy by to process First Midwest Bank</Description>
<ChangeLog>
<log Date="08/15/2016" Author="Mhinojo"> Creación </log>
<log Date="24/12/2018" Author="jmolina"> Add with(nolock) and ; </log>
<log Date="12/06/2019" Author="jhornedo"> Add with(nolock) in inner join Agent </log>
...
<log Date="03/06/2022" Author="jdarellano" Name="#6">Se cambian variables tipo tabla por tablas temporales, y se agrega esquema a tablas físicas.</log>
<log Date="03/06/2022" Author="jdarellano" Name="#7">Se cambia la forma de obtener las imágenes para cada cheque.</log>
<log Date="03/06/2022" Author="jdarellano" Name="#8">Se agrega validación para no considerar agencias de 1st Midwest (dbo.AgentsFirstMidwest).</log>
<log Date="19/10/2022" Author="adominguez" Name="#9">Se omite envio de IRD's por este canal.</log>
</ChangeLog>
*********************************************************************/
BEGIN
	BEGIN TRY
		...
		INNER JOIN dbo.Checks AS c WITH (NOLOCK) ON u.IdReference = c.IdCheck AND c.IdStatus = 20
		INNER JOIN dbo.Agent AS A WITH (NOLOCK) ON c.IdAgent = A.IdAgent AND EXISTS (SELECT 1 FROM dbo.AgentsFirstMidwest AS F WITH (NOLOCK) WHERE A.IdAgent = F.IdAgent)--#8 
		...
		INNER JOIN #UFUFD_F AS F ON F.IdReference = C.IdCheck--#7
		INNER JOIN #UFUFD_R AS F2 ON F2.IdReference = C.IdCheck--#7
		...
		INNER JOIN dbo.Agent A WITH (NOLOCK) ON A.IdAgent = C.IdAgent 
		--AND A.AgentCode IN ('5525-KS','6172-KS','4947-KS','5301-KS','7030-KY','4913-KS','5088-KS','6460-KY','5557-KY','8338-KY','5464-KY','6548-MO','4009-MO','9398-IL','11384-IL','9398-IN','11210-IL','11780-IL','11412-TN','10185-TN','11757-IL','14324-IL') --Fix Agent nolock--#3--#4--#5
		AND EXISTS (SELECT 1 FROM dbo.AgentsFirstMidwest AS F WITH (NOLOCK) WHERE A.IdAgent = F.IdAgent)--#8
		WHERE 
		--IdCheck=7116294
		--IdCheck=7115089
		--IdCheck=7115082
		...
		--IdCheck=7115076
		--IdCheck=7115075
		LTRIM(RTRIM(ISNULL(C.MicrManual,''))) != '' AND C.IdStatus = @IdStatusStandBy 
		AND C.CheckFile IS NULL 
		and C.IsIRD = 0;--#9

		UPDATE [dbo].[Checks] SET IdStatus = @IdStatusPendingGatewayResponse, IdCheckProcessorBank=4
		...
		BEGIN
			...
			BEGIN		
				...
				WHERE IdCheck IN (SELECT TOP (@BundleMaxSize - @CreditsOnBundle) IdCheck FROM #Checks WHERE IdAgent = @TempIdAgent AND BundleSequence IS NULL ORDER BY IdCheck);--#6
		...
		DECLARE @DestinationRoutingNumber varchar(100) = (SELECT [Value] FROM [Services].[ServiceAttributes] WITH (NOLOCK) WHERE [Code] = 'FIRSTMIDWESTSEND' AND [Key] = 'Field_DestinationRoutingNumber');
		DECLARE @OriginRoutingNumber varchar(100) = (SELECT [Value] FROM [Services].[ServiceAttributes] WITH (NOLOCK) WHERE [Code] = 'FIRSTMIDWESTSEND' AND [Key] = 'Field_OriginRoutingNumber');
	...
	BEGIN CATCH
		...
		INSERT INTO DBO.ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage) VALUES ('dbo.st_GetChecksToProcessFirstMidwest',GETDATE(),@ErrorMessage);
	END CATCH
END