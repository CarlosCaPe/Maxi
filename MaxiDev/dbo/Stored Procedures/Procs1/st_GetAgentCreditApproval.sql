...
CREATE PROCEDURE [dbo].[st_GetAgentCreditApproval]
(
    @Idagent int
)
as
...
IF EXISTS (SELECT 1 FROM AgentCreditApproval with(nolock) WHERE IdAgent=@Idagent AND IsApproved is null) RETURN
...
if (@BalanceActual >= @CreditActual-(@CreditActual*@PercentForLimitCredit/100) AND 
    EXISTS
        (   
            SELECT 1 FROM dbo.AgentDeposit a with(nolock) where idagent=@Idagent AND dbo.RemoveTimeFromDatetime(DepositDate) >= dbo.RemoveTimeFromDatetime([dbo].[funPastPaymentDateN](A.IdAgent,GETDATE()))--dbo.RemoveTimeFromDatetime(dbo.funLastPaymentDate(A.IdAgent,GETDATE()))
        )
    )
begin
