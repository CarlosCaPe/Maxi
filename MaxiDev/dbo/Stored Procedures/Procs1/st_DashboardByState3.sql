create Procedure [dbo].[st_DashboardByState3]
(
    @WeeksAgo int,
...
Select SUM(NumTran) as TotalOneMonthAgo,sum(AmountInDollars) TotalAmountInDollarsOneMonthAgo, IdAgent into #T3
	from(
		select SUM(1) as NumTran,sum(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0)) AmountInDollars, A.IdAgent from Transfer T
		Join Agent A on (T.IdAgent=A.IdAgent)
		...
		union all
		select SUM(1) as NumTran,sum(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0)) AmountInDollars, A.IdAgent
		...
		union all
		select SUM(1)*-1 as NumTran,(-1)*sum(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0)) AmountInDollars, A.IdAgent
		from Transfer T
		Join Agent A on (T.IdAgent=A.IdAgent)
...
Select SUM(NumTran) as TotalTwoMonthAgo,sum(AmountInDollars) TotalAmountInDollarsTwoMonthAgo, IdAgent into #T4
	from(
		...
		union all
		select SUM(1) as NumTran,sum(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0)) AmountInDollars, A.IdAgent from TransferClosed T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateOfTransfer>= @TwoMonthAgoSD and T.DateOfTransfer<@TwoMonthAgoED
				and A.AgentState=@State
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))
		...
		union all
		select SUM(1)*-1 as NumTran,(-1)*sum(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0)) AmountInDollars, A.IdAgent from Transfer T
		Join Agent A on (T.IdAgent=A.IdAgent)
		...
		union all
		select SUM(1)*-1 as NumTran,(-1)*sum(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0)) AmountInDollars, A.IdAgent from TransferClosed T
		Join Agent A on (T.IdAgent=A.IdAgent)
		where T.DateStatusChange>= @TwoMonthAgoSD and T.DateStatusChange<@TwoMonthAgoED and T.IdStatus in (22,31)
				and A.AgentState=@State
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))
