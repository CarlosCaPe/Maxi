...
CREATE PROCEDURE [dbo].[st_EvaluateKYCRuleTest]                                          
(                                       
...
Begin                                        
  ...
  --From  Transfer                                                               
  --  Where               
  --  IdPayer  =Case When Null IS null  Then IdPayer ELSE 18 END And                                         
  --  --IdPaymentType = Case When null IS not null Then 1 Else IdPaymentType End  And                                         
  --  IdCustomer = 578665 And                                           
  --  DATEDIFF (day,DateOfTransfer,GETDATE() ) <=  30-1  And                                        
  --  IdStatus Not In (22,31 ) --(25= Rejected, 16= Cancelled)                 
    
  --  --ISNULL( Case When 10 = 17 THEN SUM( AmountInDollars)  ELSE SUM( AmountInMN) END , 0) 
  --  Select *                                                                 
  --From  TransferClosed                                                               
  --  Where               
  --  IdPayer  =Case When Null IS null  Then IdPayer ELSE 18 END And                                         
  ...
  --  Select DATEDIFF (day,'2012-01-22 14:59:41.040',GETDATE() )
