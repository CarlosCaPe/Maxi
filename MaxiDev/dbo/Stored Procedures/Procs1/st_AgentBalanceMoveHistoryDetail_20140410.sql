...
create procedure [dbo].[st_AgentBalanceMoveHistoryDetail_20140410]              
(              
@IdAgent int,              
@DateFrom datetime,               
@DateTo datetime              
...
join AgentCollection AC on AC.IdAgentCollection = ACD.IdAgentCollection      
...
select 
    ...
    case when ACD.lastamounttopay=0 then 'Collect Plan Creation' else ACD.Note end as Description,
    ...
    --ACD.LastAmountToPay       
from 
    AgentCollectionDetail ACD 
inner join 
	AgentCollection AC on AC.IdAgentCollection = ACD.IdAgentCollection inner join
