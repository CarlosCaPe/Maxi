...
CREATE PROCEDURE st_findLocationByName(@busqueda VARCHAR(200), @TOP INT = NULL, @idOut INT = NULL OUT  )
AS 
/********************************************************************
...
BEGIN 
	...
	JOIN Location L ON L.idLocation = p.[KEY]
	...
	IF NOT EXISTS (SELECT 1 FROM @ExactMatch) BEGIN
		...
		SELECT idCountry, idState, idCity, LocationName FROM location WHERE CONTAINS((LocationName,AL1,AL2,AL3),@compsearch) 
		ORDER BY LocationName
		
		SELECT TOP 1 @idOut = idLocation FROM Location WHERE CONTAINS((LocationName,AL1,AL2,AL3),@compsearch) 	ORDER BY LocationName
