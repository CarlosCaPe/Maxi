CREATE PROCEDURE [dbo].[st_GetChecksToProcessBankOfTexasNV]
AS
/********************************************************************
<Author>Miguel Hinojo</Author>
<app>Maxi Host Manager Service</app>
<Description>Get checks that are on standy by to process Bank of Texas</Description>
<ChangeLog>
<log Date="08/09/2016" Author="Mhinojo"> Creación </log>
<log Date="19/12/2018" Author="jmolina"> Add with(nolock) and ; </log>
<log Date="01/06/2022" Author="jdarellano" Name="#1">Se cambian variables tipo tabla por tablas temporales, y se agrega esquema a tablas físicas.</log>
<log Date="01/06/2022" Author="jdarellano" Name="#2">Se cambia la forma de obtener las imágenes para cada cheque.</log>
<log Date="01/06/2022" Author="jdarellano" Name="#3">Se agrega validación para no considerar agencias de 1st Midwest (dbo.AgentsFirstMidwest).</log>
<log Date="26/12/2022" Author="adominguez" Name="#4">Se agregan IRD's para 1st Midwest Nevada(dbo.AgentsFirstMidwestNV).</log>
</ChangeLog>
*********************************************************************/
BEGIN
	BEGIN TRY
		...
		FROM dbo.UploadFiles AS u WITH (NOLOCK)
		INNER JOIN dbo.Checks AS c WITH (NOLOCK) ON u.IdReference = c.IdCheck AND c.IdStatus = 20
		INNER JOIN dbo.Agent AS A WITH (NOLOCK) ON c.IdAgent = A.IdAgent AND EXISTS (SELECT 1 FROM dbo.AgentsFirstMidwestNV AS F WITH (NOLOCK) WHERE A.IdAgent = F.IdAgent)
		WHERE u.IdDocumentType = 69 AND u.[FileName] LIKE REPLACE('00000000-0000-0000-0000-000000000000', '0', '[0-9a-fA-F]') 
		and C.IsIRD = 1;
		...
		DECLARE @DestinationRoutingNumber varchar(100) = (SELECT [Value] FROM [Services].[ServiceAttributes] WITH (NOLOCK) WHERE [Code] ='BANKOFTEXASNVSEND' AND [Key] = 'Field_DestinationRoutingNumber');
		DECLARE @OriginRoutingNumber varchar(100) = (SELECT [Value] FROM [Services].[ServiceAttributes] WITH (NOLOCK) WHERE [Code] ='BANKOFTEXASNVSEND' AND [Key] = 'Field_OriginRoutingNumber');
		...
		FROM #Credits C--#1
		INNER JOIN dbo.Agent A WITH (NOLOCK) ON A.IdAgent = C.IdAgent AND A.AgentState = 'NV';--#1
	...
	BEGIN CATCH
		...
		INSERT INTO dbo.ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage) VALUES ('dbo.st_GetChecksToProcessBankOfTexasNV',GETDATE(),@ErrorMessage);                                                                                            
	END CATCH
END