CREATE procedure [dbo].[st_CreateAgentApplication]
(
    ...
    ,@AccountNumberCommission nvarchar(50)
    ,@RoutingNumberCommission nvarchar(50)
    ,@IdCounty int
    ,@IdOwner int
	...
	,@ComplianceOfficerPlaceOfBirth nvarchar(250) = NULL
	,@MailCheckTo nvarchar(20) = NULL
	,@TypeTaxID INT = NULL
...
<log Date="26/12/2022" Author="maprado">Se agrega AgentApp_Website, OwnerIdStateEmission, OwnerIdCountryEmission, ComplianceOfficerPlaceOfBirth, MailCheckTo</log>
</ChangeLog>
********************************************************************/
SET NOCOUNT ON;
...
begin
	...
	select @OwnerIdTypeTemp = ot.IdType, @OwnerIdNumberTemp = ot.IdNumber, @OwnerIdExpirationDateTemp = ot.IdExpirationDate from [Owner] ot with(nolock) where IdOwner = @IdOwner

	if @OwnerIdTypeTemp <> @OwnerIdType or @OwnerIdNumberTemp <> @OwnerIdNumber or @OwnerIdExpirationDateTemp <> @OwnerIdExpirationDate
	begin
		update Owner set IdType = @OwnerIdType, IdNumber = @OwnerIdNumber, IdExpirationDate = @OwnerIdExpirationDate, DateofLastChange = GETDATE(), EnterByIdUser = @EnterByIdUser where IdOwner = @IdOwner;
	end
