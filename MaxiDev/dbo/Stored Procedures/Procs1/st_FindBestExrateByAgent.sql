...
*********************************************************************/
CREATE Procedure [dbo].[st_FindBestExrateByAgent]  
(  
    @IdAgent int,  
    @IdPayer Int,  
...
Select top 1 @ExRate=(CASE WHEN @IsSwitchSpecExRateGroup = 1 AND R4.IdPayer = E.IdPayer then R4.RefExRateByGroup else
	ISNULL(R1.RefExRate,ISNULL(R2.RefExRate,ISNULL(R3.RefExRate,0))) END
        ...
        + CASE WHEN ISNULL(C.IdSpread ,0)>0 THEN (SELECT ISNULL(MAX(SD.SpreadValue),0) FROM SpreadDetail SD (NOLOCK) WHERE SD.IdSpread = C.IdSpread/*AND @SpreadValue BETWEEN SD.FromAmount AND SD.ToAmount*/) --29012019-azavala
        ...
        --+c.SpreadValue+case when B.EndDateSpread>GETDATE() Then B.Spread Else 0 End) ,
        @PayerLogo=Isnull(E.PayerLogo,'NotSelected.jpg'),@PayerName=E.PayerName 
FROM AgentSchema B
...
Where B.IdAgent=@IdAgent and B.IdCountryCurrency=@IdCountryCurrency and D.IdPayer=@IdPayer  and Isnull(E.Idgenericstatus,0)=1 and D.IdPaymentType in (1,4) and B.IdGenericStatus=1
Order by CASE WHEN @IsSwitchSpecExRateGroup = 1 AND R4.IdPayer = E.IdPayer then R4.RefExRateByGroup else
	...
	+CASE WHEN ISNULL(C.IdSpread ,0)>0
						THEN (SELECT ISNULL(MAX(SD.SpreadValue),0) FROM SpreadDetail SD (NOLOCK) WHERE SD.IdSpread = C.IdSpread/*AND @SpreadValue BETWEEN SD.FromAmount AND SD.ToAmount*/) --29012019-azavala
						WHEN @IsSwitchSpecExRateGroup = 1 AND R4.IdPayer = E.IdPayer then 0
						ELSE ISNULL(C.SpreadValue,0) 
						END
...
End Try
Begin Catch
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('[dbo].[st_FindBestExrateByAgent]',Getdate(),ERROR_MESSAGE())    
End Catch
