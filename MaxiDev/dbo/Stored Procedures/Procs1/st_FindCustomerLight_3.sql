CREATE procedure [dbo].[st_FindCustomerLight_3]
@name varchar(40),
...
CREATE TABLE  #customerPivot  (IdCustomer INT,IdAgentCreatedBy INT )
...
--Descomentado para mejor performance
IF @name !=''
SET @SQL=@SQL +'	    AND [C].[Name] like @nameT '
IF @firstLastName != ''
SET @SQL=@SQL +'		AND [C].[FirstLastName] like @firstLastNameT'
IF @secondLastName !=''
SET @SQL=@SQL +'		AND [C].[SecondLastName] like @secondLastNameT'
IF @cellular !=''
SET @SQL=@SQL +'		AND [C].[CelullarNumber] like @cellularT'
IF @phone !=''
SET @SQL=@SQL +'		AND [C].[PhoneNumber] like @phoneT'
