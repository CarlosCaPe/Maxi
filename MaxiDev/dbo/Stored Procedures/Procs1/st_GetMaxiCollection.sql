CREATE Procedure [dbo].[st_GetMaxiCollection]
(
...
select @IsUser=IsUser from [CollectionUsers] where iduser=@IdUser
...
FROM 
    #Collect c
JOIN 
    dbo.AgentStatus s ON c.IdAgentStatus=s.IdAgentStatus
GROUP BY 
...
where 
	(
		c.idagent in (select idagent from MaxiCollectionAssign where iduser=@IdUser and DateOfAssign=@CollectDate)
		or
		c.idagent in (select idagent from collection.Group_Assigment where IdUserAssign=@IdUser)
	)
GROUP BY 
    c.IdAgentStatus,AgentStatus
HAVING
