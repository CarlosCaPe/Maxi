...
CREATE procedure [dbo].[st_GetBeneficiaryTransactionHistory]  
@IdTransfer int 
...
set @IdBeneficiary = ( select IdBeneficiary from [Transfer] with(nolock) where IdTransfer =@IdTransfer  
       union  
      select IdBeneficiary from TransferClosed with(nolock) where IdTransferClosed =@IdTransfer)  
