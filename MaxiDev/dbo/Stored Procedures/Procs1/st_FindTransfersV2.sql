...
CREATE    PROCEDURE [dbo].[st_FindTransfersV2]   
...
BEGIN TRY
		...
		IF @TransferFolio IS NOT NULL
			SET @SearchOption = 1
		ELSE IF @BeginDate IS NOT NULL AND @EndDate IS NOT NULL AND @ExistsParametersXml = 0
		AND @Customer IS NULL AND @Beneficiary IS NULL AND @IdStatus IS NULL AND @IdPayer IS NULL
		AND @IdCurrency IS NULL
			SET @SearchOption = 2
		...
		BEGIN
			...
			WHERE T.IdAgent =@IdAgent
				AND T.[IdCustomer] = (SELECT IdCustomer FROM Customer WITH (NOLOCK)  WHERE Contains(CelullarNumber,@FTSCustomerPhone) OR Contains(CelullarNumber,@FTSCustomerPhoneNotFormatted));
		...
		BEGIN
			...
			SELECT IdTransfer 
			FROM [dbo].[Transfer] T WITH (NOLOCK) 
			WHERE T.IdAgent =@IdAgent AND T.[IdCustomer] = (SELECT vp.IdCustomer FROM CardVIP vp  WITH (NOLOCK) 
		...
		IF @IsTimeForVerifyHold = 1
		 ...
		 CREATE TABLE #tmpMessages (idTransfer INT NULL, EnterDate DATETIME NULL, RAWMESSAGE NVARCHAR(max) NULL,IdTransferNote INT NULL,IdGenericStatus INT NULL  )
		...
		SELECT IdTransfer, EnterDate, RAWMESSAGE, IdTransferNote, IdGenericStatus FROM (
		...
		SELECT IdTransfer, EnterDate, RAWMESSAGE, IdTransferNote, IdGenericStatus FROM (
		 ...
		 SELECT 
			 ...
			 TD.IdTransferClosed AS IdTransfer,
	   			TN.EnterDate, 
	  		 RAWMESSAGE = Convert(NVARCHAR(max),'{"IdTransfer":'+Convert(VARCHAR,tmp.IdTransfer)+',"AgentCode":"","AgentName":"","Folio":"'+Convert(varchar,tmp.Folio)+'","ClaimCode":"","DateOfTransfer":"'+Convert(varchar,tmp.DateOfTransfer,127)+'","CustomerName":"","Note":"'+isnull(TN.note,'')+'","Requirement":[{"IdComplianceProduct":0,"Name":null,"IdStatus":0,"NameEs":"Motivo: '+rc.Reason+'","NameEn":"Reason: '+rc.ReasonEn+'"}]}') ,
	  		 TN.IdTransferClosedNote AS IdTransferNote, 
			 1 AS IdGenericStatus
		...
		FROM #tmp A
			...
			LEFT JOIN CardVIP CV ON CV.IdCustomer = A.idCustomer
