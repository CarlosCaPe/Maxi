CREATE PROCEDURE [dbo].[st_GetTransfersHoldWithFileReserved]  
(  
 @IdUser INT, 
 @IdTransfer INT  	
)
		...
		while exists (select 1 from #tempAllTrans)
		 begin
		   ...
		   order by DateOfTransfer
		    ...
		    where  
		     IdTransfer = @IdTransfertemp
		    if (select count(*) from TransferByHoldReserved with(nolock) where IdUser = @IdUser and IdStatusReserved = 1) > 5
		      break          
		   end
