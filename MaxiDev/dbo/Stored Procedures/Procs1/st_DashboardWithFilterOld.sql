CREATE Procedure [dbo].[st_DashboardWithFilterOld]
(
    @WeeksAgo int,
...
--#tempT1
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempT1 from Transfer T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
		where T.DateOfTransfer>= @NowStart and T.DateOfTransfer<@Now
                ...
                and t.idpayer=isnull(@idpayer,t.idpayer)
--#tempT2
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempT2 from TransferClosed T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
		where T.DateOfTransfer>= @NowStart and T.DateOfTransfer<@Now
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))
...
--#tempT3
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull((((case when TA.IdTransfer is null and T.IdStatus=22 then 0 else Fee end)-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempT3 from Transfer T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
...
--#tempT4
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull((((case when TA.IdTransfer is null and T.IdStatus=22 then 0 else Fee end)-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempT4 from TransferClosed T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
...
Select SUM(NumTran) as TotalToday,sum(AmountInDollars) TotalAmountInDollarsToday, AgentState into #T2
	from(		
        select SUM(1) as NumTran,sum(AmountInDollars-(case when IdAgentCollectType=1 then 0 else isnull(AmountInDollarsForCommission*FactorNew,0) end)-(isnull(CommissionNew,0))) AmountInDollars, AgentState
        from #tempT1 T
        left join bankcommission b on b.DateOfBankCommission=DateOfCommission and b.active=1
        left join payercommission p on p.DateOfpayerCommission=DateOfCommission and p.idpayer=t.idpayer and p.idpaymenttype=t.idpaymenttype	and p.active=1
		group by AgentState
        ...
        select SUM(1)*-1 as NumTran,sum(AmountInDollars-(case when IdAgentCollectType=1 then 0 else isnull(AmountInDollarsForCommission*FactorNew,0) end)-(isnull(CommissionNew,0)))*-1 AmountInDollars, AgentState
        from #tempT3 T
        left join bankcommission b on b.DateOfBankCommission=DateOfCommission and b.active=1
        left join payercommission p on p.DateOfpayerCommission=DateOfCommission and p.idpayer=t.idpayer and p.idpaymenttype=t.idpaymenttype	and p.active=1
		group by AgentState
...
Select SUM(NumTran) as TotalOneMonthAgo,sum(AmountInDollars) TotalAmountInDollarsOneMonthAgo, AgentState  into #T3 
	from(
		select Numtran, AmountInDollars, A.AgentState from DashboardForGatewayCountry T
		Join Agent A on (T.IdAgent=A.IdAgent)
...
--#tempM1
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempM1 from Transfer T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
		where T.DateOfTransfer>= @CurrentMonthSD and T.DateOfTransfer<@CurrentMonthED
                ...
                and t.idpayer=isnull(@idpayer,t.idpayer)
--#tempM2
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull(((fee-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempM2 from TransferClosed T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
		where T.DateOfTransfer>= @CurrentMonthSD and T.DateOfTransfer<@CurrentMonthED
				and (@IsAllSeller=1 Or (A.IdUserSeller = @IdUserSeller or A.IdUserSeller in (select IdSeller from #SellerSubordinates)))
...
--#tempM3
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull((((case when TA.IdTransfer is null and T.IdStatus=22 then 0 else Fee end)-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempM3 from Transfer T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
        left join dbo.TransferNotAllowedResend TA on T.IdTransfer=TA.IdTransfer  
...
--#tempM4
select a.IdAgentCollectType,t.idpayer,t.idpaymenttype,Round(isnull((((case when TA.IdTransfer is null and T.IdStatus=22 then 0 else Fee end)-agentcommission) + (((ReferenceExRate-ExRate)*AmountInDollars)/ReferenceExRate)),0),2) AmountInDollars, AmountInDollars AmountInDollarsForCommission, A.AgentState,[dbo].[RemoveTimeFromDatetime](DATEADD(dd,-(DAY(DateOfTransfer)-1),DateOfTransfer)) DateOfCommission into #tempM4 from TransferClosed T with (nolock)
		Join Agent A on (T.IdAgent=A.IdAgent)
        join countrycurrency c on t.idcountrycurrency=c.idcountrycurrency
        left join dbo.TransferNotAllowedResend TA on T.IdTransferClosed=TA.IdTransfer  
		where T.DateStatusChange>= @CurrentMonthSD and T.DateStatusChange<@CurrentMonthED and T.IdStatus in (22,31)
