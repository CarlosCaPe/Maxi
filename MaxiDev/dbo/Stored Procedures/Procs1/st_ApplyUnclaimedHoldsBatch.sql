CREATE PROCEDURE [dbo].[st_ApplyUnclaimedHoldsBatch] 
(
	@note varchar(max)
	, @EnterByIdUser INT
	, @data xml
...
AS BEGIN
		...
		BEGIN 
		  ...
		  BEGIN 
			 ...
			 FROM  @XMLTable WHERE IdStatus = 27;
				...
				exec [dbo].[st_AddNoteToTransfer] @IdTrasnfer, @EnterByIdUser, @note,1,@TransferDetail OUTPUT,@HasError OUTPUT,@Message OUTPUT;			
		  ...
		  BEGIN 
			 ...
			 FROM  @XMLTable WHERE IdStatus != 27;
				...
				INSERT INTO [MAXILOG].[dbo].[LogForUnclaimedHoldsBatch]
					(
						[IdUser],
						[IdStatus],
						[ClaimCode],
	...
	BEGIN CATCH 
		...
		Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_ApplyUnclaimedHoldsBatch',Getdate(),@ErrorMessage);
		set @HasError = 1;
