...
CREATE Procedure [dbo].[st_CreateComplianceNotificationSignatureHold]
( 
	@IdAgent INT,
	@IdTransfer INT,
...
<ChangeLog>
<log Date="26/01/2018" Author="jdarellano" Name="#1">Performance: se elimina "TOP 1" de "EXISTS".</log>
</ChangeLog>
*********************************************************************/
SET NOCOUNT ON
...
BEGIN TRY
	...
	select 
		...
		from [dbo].[Transfer] T with(nolock)
			...
			join [dbo].[TransferHolds] H with(nolock) on T.IdTransfer=H.IdTransfer and H.IdStatus=3 and H.IsReleased is null
		where T.IdStatus = 41 
	...
	SELECT 
		@NoteEs= [Name]
		, @NoteEn=nameen 
	FROM [dbo].[ComplianceProducts] WITH(nolock) WHERE  IdComplianceProduct = @IdComplianceProduct;
	...
	while Exists(SELECT 1 FROM  @Transfers)--#1
	BEGIN
		DECLARE 
		...
		IF NOT EXISTS(SELECT 1 FROM [dbo].[AgentUser] AS A with(nolock) --#1
						INNER JOIN [dbo].[UsersSession] AS US with(nolock)  ON A.IdUser = US.IdUser
		...
		BEGIN
			...
			IF EXISTS(SELECT 1 FROM [dbo].[Transfer] WITH(NOLOCK)--#1
		...
		BEGIN
			...
			while Exists(SELECT 1 FROM  @Transfers WHERE IdAgent = @IdAgent)--#1
			BEGIN
		...
		Begin --Agent es de tipo PC, enviar notificación
			...
			EXEC @Update = [dbo].[st_GetMessagesExisting] 
								@IdAgent,	
...
Begin Catch
	 ...
	 Insert into [dbo].[ErrorLogForStoreProcedure] (StoreProcedure,ErrorDate,ErrorMessage)Values('st_CreateComplianceNotificationSignatureHold',Getdate(),@ErrorMessage) 
End Catch 
