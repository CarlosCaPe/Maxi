...
CREATE PROCEDURE [dbo].[st_BulkSuggestAgentCredit]
(
    @AgentsData XML,
    @IsApprove bit,    
    @IdUser INT,
...
BEGIN TRY
    ...
    BEGIN
	   ...
	   begin
		  ...
		  update agent 
			 set creditamount=@CreditLimitSuggested
				, dateoflastchange=getdate()
...
BEGIN CATCH
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_BulkSuggestAgentCredit',Getdate(),@ErrorMessage);
END CATCH