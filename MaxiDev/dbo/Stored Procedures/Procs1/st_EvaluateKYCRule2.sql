...
CREATE PROCEDURE [dbo].[st_EvaluateKYCRule2]
(
...
set @MaxTimeInDays=isnull(@MaxTimeInDays,0)
        ...
        Select AmountInDollars,AmountInMN,DateOfTransfer
		...
		IdStatus Not In (22,31 ) --(25= Rejected, 16= Cancelled)
        union all
        select ReceiptAmount AmountInDollars,0 AmountInMN,PaymentDate DateOfTransfer from BillPaymentTransactions where customerid=@IdCustomer and status=1
        --)t
