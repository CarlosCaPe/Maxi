...
CREATE Procedure [dbo].[st_SaveOwner] (
	@idOwner int,
    @name varchar(MAX),
...
Begin Try
	...
	if(exists (select top  1 1 from Owner where SSN = @ssn and IdOwner != @idOwner))
		begin 
			Set @HasError=1
		...
		begin 
			if(@idOwner > 0)
				begin
					if(exists(select top 1 1 from Owner where IdOwner = @idOwner))
						begin 
...
Begin Catch        
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_SaveOwner',Getdate(),@ErrorMessage)
