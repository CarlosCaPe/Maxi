CREATE procedure [dbo].[st_ResponseReturnCodeServiCentro]                                              
(                                              
    @IdGateway  int,                                              
...
Insert INTO [MAXILOG].[dbo].[ServiCentroResponseLog] values (getdate(),@Claimcode,@ReturnCode,@ReturnCodeType,@IdStatusAction,@Description,@XmlValue)                      
...
Begin                                            
	...
	Begin                                           
		...
		Begin
            exec st_SavePayInfoServiCentro @IdGateway,@IdTransfer,@Claimcode,@XmlValue
        End  
...
Begin          
    ...
    begin                             
        set @Description = 'incidencia ' + @Description
        Exec st_SimpleAddNoteToTransfer  @IdTransfer,@Description                                              
    end       
End                              
Set @IsCorrect=1
...
BEGIN CATCH
	Declare @ErrorMessage nvarchar(max)                                                                                             
	Select @ErrorMessage=ERROR_MESSAGE()                                             
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_ResponseReturnCodeServiCentro',Getdate(),@ErrorMessage)                                                                                            
END CATCH