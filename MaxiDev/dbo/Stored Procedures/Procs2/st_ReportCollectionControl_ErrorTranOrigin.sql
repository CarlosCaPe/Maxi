CREATE procedure [dbo].[st_ReportCollectionControl_ErrorTranOrigin]  
@BeginDate datetime,                        
@EndDate datetime                        
as                        
...
group by t0.idAgent      
 ...
 where DateOfTransfer>=@BeginDate and DateOfTransfer<@EndDate    and A.IdTransferClosed not in (16360189,
16381696,
16381700,
16381717,
16400681,
...
16401028)              
 ...
 from TransferClosed A  Join #agentTemp B on (A.IdAgent=B.IdAgent)                
 where DateStatusChange>=@BeginDate and DateStatusChange<@EndDate              
 and IdStatus in (22)  and A.IdTransferClosed not in (16360189,
16381696,
16381700,
16381717,
16400681,
...
--Declare @BeginDateSpecialCommission datetime = DATEADD(day,(day(@BeginDate)*-1)+1,@BeginDate)
 ...
 from  checks 
 where idstatus=30  and DateOfMovement>= @BeginDate AND DateOfMovement<@EndDate
 group by idagent	
