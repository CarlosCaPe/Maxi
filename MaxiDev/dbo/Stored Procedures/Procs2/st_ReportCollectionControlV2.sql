...
CREATE procedure [dbo].[st_ReportCollectionControlV2]
@BeginDate datetime,                      
@EndDate datetime                      
as                      
...
select  
    AC.IDAGENT,   
    (select top 1 ActualAmountToPay from agentcollectiondetail where LastAmountToPay=0 and CreationDate <=@EndDate and idagentcollection=ac.IdAgentCollection  order by IdAgentCollectionDetail desc) cp,    
    (select top 1 ActualAmountToPay from agentcollectiondetail where LastAmountToPay=0 and CreationDate <=@EndDate and idagentcollection=ac.IdAgentCollection  order by IdAgentCollectionDetail desc)-d.ActualAmountToPay cpm,
    --(select top 1 ActualAmountToPay from agentcollectiondetail where LastAmountToPay=0 and CreationDate <=@BeginDate+1 and idagentcollection=ac.IdAgentCollection  order by IdAgentCollectionDetail desc) cp,    
    --(select top 1 ActualAmountToPay from agentcollectiondetail where LastAmountToPay=0 and CreationDate <=@BeginDate+1 and idagentcollection=ac.IdAgentCollection  order by IdAgentCollectionDetail desc)-d.ActualAmountToPay cpm,
    d.ActualAmountToPay fcp,
    d.DateofLastChange 
into 
    #CP
from 
...
join 
    AgentCollection AC on d.idagentcollection=ac.IdAgentCollection --and ac.idagent=1241
where D.DateofLastChange<@EndDate
order by IdAgentCollectionDetail desc                 
 ...
 Select               
   ...
   CurrentBalance    ,
   isnull((select top 1 CP from #CP where idagent=t.idagent order by DateofLastChange desc),0) InitialCP,
   isnull((select top 1 cpm from #CP where idagent=t.idagent order by DateofLastChange desc),0) CPMovements,
   isnull((select top 1 fcp from #CP where idagent=t.idagent order by DateofLastChange desc),0) FinalCP,
   CurrentBalance + 
   isnull((select top 1 fcp from #CP where idagent=t.idagent order by DateofLastChange desc),0) ConsolidatedDebt
from  
    #tempFinal t               
order 
    by AgentOrder,Agent
