CREATE PROCEDURE [dbo].[st_KYCAnalysisReport] (
             @DateFrom DATETIME,
             @DateTo DATETIME,
             @SearchType INT = 1,
             @Satatus INT= NULL,
...
<ChangeLog>
<log Date="24/02/2018" Author="jdarellano" Name="#1">Performance: se agrega "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITED".</log>
<log Date="26/02/2018" Author="jmolina" Name="#2">Performance: se agrega "#CTE_TRANS" para sustituir el "WITH CTE_TRANS".</log>
</ChangeLog>
*********************************************************************/
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;--#1
	...
	SET @Message = ''
	BEGIN TRY
		IF OBJECT_ID('tempdb..#CTE_TRANS') IS NOT NULL DROP TABLE #CTE_TRANS --#2
		CREATE TABLE #CTE_TRANS (
			...
			[CustomerCelullarNumber] [nvarchar](max) NULL,
			[String B] [nvarchar](max) NULL,
			...
			[IdentificationIdCountry] [nvarchar](max) NOT NULL,
			...
			[Date] [datetime] NULL,
			[CityName1] [nvarchar](max) NOT NULL,
			[StateName1] [nvarchar](max) NOT NULL,
			[KYCHold] [int] NOT NULL,
			[DenyListHold] [int] NOT NULL,
			[RejectedHold] [nvarchar](max) NOT NULL,
			[CountyInfo] [nvarchar](max) NOT NULL,
			[RequireSSN] [bit] NULL,
			[StateRequired] [bit] NULL,
	...
	--- =========>>
		...
		BEGIN
		--WITH CTE_TRANS  AS (
			INSERT INTO #CTE_TRANS ([AmountRequiredToAskId], [AgentCode], [AgentZipcode], [Folio], [DateOfTransfer], [AmountInDollars], [StatusName], [DepositAccountNumber], [ClaimCode], [IdCustomer], [String C], [CustomerName], [CustomerFirstLastName]
								  , [CustomerSecondLastName], [CustomerAddress], [CustomerCity], [CustomerState], [CustomerZipcode], [CustomerPhoneNumber], [CustomerCelullarNumber], [String B], [BeneficiaryName], [BeneficiaryFirstLastName], [BeneficiarySecondLastName]
								  , [RecipientAddress], [PayerName], [IdBranch], [CityName], [StateName], [CountryName], [UserName], [Name], [IdentificationIdCountry], [IdentificationNumber], [SSNumber], [BornDate], [Occupation], [BranchName], [Date]
								  , [CityName1], [StateName1], [KYCHold], [DenyListHold], [RejectedHold], [CountyInfo], [RequireSSN], [StateRequired], [CustomerIdentificationIdState])--#2
			SELECT * 
			  FROM (
			SELECT
	...
	BEGIN CATCH
		...
		INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES('st_KYCAnalysisReport: '+ ' Linea: '+CONVERT(VARCHAR,ERROR_LINE()),GETDATE(),@ErrorMessage)
	END CATCH
END
