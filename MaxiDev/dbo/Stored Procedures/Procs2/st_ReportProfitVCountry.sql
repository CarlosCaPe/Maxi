CREATE  procedure [dbo].[st_ReportProfitVCountry]
(
...
--#2
    Insert into #TempAgents (IdAgent,Idcountry, IdCountryCurrency)
	    ...
	    Select t.IdAgent, 0 IdCountry, 0 IdCountryCurrency  
		  from TransferClosed AS t-- WITH (nolock,INDEX(ixDateOfTransfer))
	     where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		   and DateOfTransfer>@StartDate 
		   and DateOfTransfer<@EndDate
		    OR (
...
--#2
    ...
    sum(CASE WHEN t.IdPaymentMethod = 2 THEN 1 ELSE 0 END) AS DCTr1,
    sum(t.OperationFee - t.Discount) AS MerchantFee1,
    ...
    sum(CASE WHEN t.IdPaymentMethod = 2 THEN 1 ELSE 0 END) AS DCTr2,
    sum(t.OperationFee - t.Discount) AS MerchantFee2,
    ...
    sum(CASE WHEN t.IdPaymentMethod = 2 THEN 1 ELSE 0 END) AS DCTr3,
    ...
    sum(CASE WHEN t.IdPaymentMethod = 2 THEN 1 ELSE 0 END) AS DCTr4,
    ...
    sum(CASE WHEN T.IdPaymentMethod = 2 THEN 1 ELSE 0 END) AS DCTr5,
    ...
    sum(CASE WHEN T.IdPaymentMethod = 2 THEN 1 ELSE 0 END) AS DCTr6,
    ...
    FeeCanR = isnull(IncomeFeeRej1, 0) + isnull(IncomeFee2Rej, 0) + isnull(IncomeFeeCancelLikeReject1,0) + isnull(IncomeFeeCancelLikeReject2,0),
    CashDiscount = isnull(CashDiscount1, 0) + isnull(CashDiscount2, 0),
    DCTran = isnull(DCTr1, 0) + isnull(DCTr2, 0),
    NetFee = isnull(NetFee1, 0) + isnull(NetFee2, 0),
    MerchantFee = isnull(MerchantFee1, 0) + isnull(MerchantFee2, 0)
