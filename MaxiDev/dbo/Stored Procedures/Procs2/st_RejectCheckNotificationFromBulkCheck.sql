...
CREATE PROCEDURE [dbo].[st_RejectCheckNotificationFromBulkCheck]
(
    @IdCheck int,
    @EnterByIdUser int,
    @Note nvarchar(MAX)        
...
BEGIN CATCH    
	...
	INSERT INTO ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_RejectCheckNotificationFromBulkCheck : ' ,Getdate(), ISNULL(@rawMessage,'') +  @ErrorMessage)                                                                                            
END CATCH
