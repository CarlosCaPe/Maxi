CREATE procedure [dbo].[st_ResponseReturnCodeIntermex]                                      
(                                      
    @IdGateway  int,                                      
...
Insert into  IntermexResponseLog values (getdate(),@Claimcode,@ReturnCode,@ReturnCodeType,@IdStatusAction,@Description,@XmlValue)              
...
Begin
	...
	begin
		...
		Begin
            exec st_SavePayInfoIntermex @IdGateway,@IdTransfer,@Claimcode,@XmlValue
        End
...
BEGIN CATCH
	Declare @ErrorMessage nvarchar(max)
	Select @ErrorMessage=ERROR_MESSAGE()
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_ResponseReturnCodeIntermex',Getdate(),@ErrorMessage)
END CATCH