CREATE PROCEDURE [dbo].[st_ResponseReturnCode24Xoro]
(                                
    @IdGateway			INT,
...
BEGIN
	...
	INSERT INTO [MAXILOGdev].[dbo].[24XoroResponseLog]
	VALUES (GETDATE(), @IdTransfer, @Claimcode, @ReturnCode, @ReturnCodeType, @NextIdStatus, @LogDescription, @XmlValue)

	IF ((@ReturnCodeType IN (1, 2,3)) OR (@IdPaymentType = 2 AND ISNULL(@NextIdStatus, 0) NOT IN (30, 0)))
	BEGIN
		...
		IF EXISTS(SELECT 1 FROM #temp)
			SELECT @TempStr = COALESCE(@TempStr + ';', '') + variable FROM #temp
		ELSE 
			SET @TempStr = CONVERT(NVARCHAR(MAX), @XmlValue)
