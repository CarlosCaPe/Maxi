CREATE PROCEDURE [dbo].[st_ResponseReturnCodeBalsas]
(                                
    @IdGateway			INT,
...
BEGIN
	...
	-- Resolve Abbreviation Return Codes

	SET @ReturnCode = CASE @ReturnCode 
		WHEN 'RECIBIDO' THEN '1'
		WHEN 'RECIBIDA CON ANTERIORIDAD' THEN '2'
		WHEN 'NUMERO DE CAMPOS INCORRECTO' THEN '3'
		WHEN 'CLAVE DE MONEDA INCORRECTA' THEN '4'
		WHEN 'CLAVE SUCURSAL INCORRECTO' THEN '5'
		WHEN 'MONTO ORDEN INCORRECTO' THEN '6'
		WHEN 'CANCELADA CON ANTERIORIDAD' THEN '7'
		WHEN 'NO EXISTENTE PARA CANCELACION' THEN '8'
		WHEN 'PAGADA CON ANTERIORIDAD' THEN '9'
		ELSE @ReturnCode
	END
	...
	INSERT INTO [MAXILOG].[dbo].BalsasResponseLog
	...
	ELSE IF @NextIdStatus = 30  -- Paid
		EXEC st_SavePayInfoBalsas @IdGateway, @IdTransfer, @Claimcode, @XmlValue
