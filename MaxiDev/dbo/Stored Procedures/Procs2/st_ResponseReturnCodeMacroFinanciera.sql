CREATE procedure [dbo].[st_ResponseReturnCodeMacroFinanciera]
(                                              
    @IdGateway  int,                                              
...
BEGIN TRY
	...
	Insert into [MAXILOG].[dbo].MacroFinancieraResponseLog values (getdate(),@Claimcode,@ReturnCode,@ReturnCodeType,@IdStatusAction,@Description,@XmlValue)                      
	...
	Begin                  
	 ...
	 Set @Description=isnull(@Description,'')+' '+@str                  
	End                    
	...
	Begin                                            
		...
		Begin                                           
			...
			Begin
				exec st_SavePayInfoMacroFinanciera @IdGateway,@IdTransfer,@Claimcode,@XmlValue
			End  
	...
	Begin        
		if @ReturnCode!='999'                     
		begin
			Select @Description='Return code UNKNOWN:'+@ReturnCode+' '+@Description                              
			Exec st_SimpleAddNoteToTransfer  @IdTransfer,@Description                              
		end
		else
		begin
			set @Description = 'Llamada ' + @Description
			Exec st_SimpleAddNoteToTransfer  @IdTransfer,@Description                                              
		end    
	End                              
	Set @IsCorrect=1
...
BEGIN CATCH
	Declare @ErrorMessage nvarchar(max)
	Select @ErrorMessage=ERROR_MESSAGE()
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_ResponseReturnCodeMacroFinanciera',Getdate(),@ErrorMessage)
END CATCH