CREATE procedure [dbo].[st_ReportProfitVCountry2]
(
...
--Set nocount on
	...
	SET @SUPERTEXT='@IdCountryCurrency='+isnull(Convert(VARCHAR,@IdCountryCurrency),'NULL')
		+'@StartDate='+isnull(Convert(VARCHAR,@StartDate),'NULL')
		+'@EndDate='+isnull(Convert(VARCHAR,@EndDate),'NULL')
		+'@IdUserSeller='+isnull(Convert(VARCHAR,@IdUserSeller),'NULL')
		+'@IdUserRequester='+isnull(Convert(VARCHAR,@IdUserRequester),'NULL')
		+'@State='+isnull(Convert(VARCHAR,@State),'NULL')
		+'@Type='+isnull(Convert(VARCHAR,@Type),'NULL')
...
begin
    Insert into #TempAgents (IdAgent,Idcountry, IdCountryCurrency)
	    Select Distinct t.IdAgent, 0 IdCountry, 0 IdCountryCurrency from Transfer t (nolock)    
	    ...
	    Union
	    Select t.IdAgent, 0 IdCountry, 0 IdCountryCurrency  from TransferClosed t  (nolock)    
	    where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		    and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate
	    Union
	    Select t.IdAgent, 0 IdCountry, 0 IdCountryCurrency  from TransferClosed t  (nolock)    
...
begin 
        Insert into #TempAgents (IdAgent,Idcountry,IdCountryCurrency)
	    Select Distinct t.IdAgent, IdCountry, 0 IdCountryCurrency   from Transfer t (nolock)    
	    ...
	    where t.IdCountryCurrency= case when @IdCountryCurrency=0 Then t.IdCountryCurrency Else @IdCountryCurrency End 
		    and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate
	    Union
	    Select t.IdAgent, IdCountry, 0 IdCountryCurrency   from TransferClosed t  (nolock)    
	    where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		    and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate
	    Union
	    Select t.IdAgent, IdCountry, 0 IdCountryCurrency   from TransferClosed t  (nolock)    
...
begin 
        Insert into #TempAgents (IdAgent,Idcountry,IdCountryCurrency)
	    Select Distinct t.IdAgent, 0 IdCountry, t.IdCountryCurrency   from Transfer t (nolock)    
        join CountryCurrency cc (Nolock) on t.IdCountryCurrency=cc.IdCountryCurrency
	    where t.IdCountryCurrency= case when @IdCountryCurrency=0 Then t.IdCountryCurrency Else @IdCountryCurrency End 
		    and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate
	    Union
	    Select t.IdAgent, 0 IdCountry, t.IdCountryCurrency   from TransferClosed t  (nolock)    
	    where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		    and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate
	    Union
	    Select t.IdAgent, 0 IdCountry, t.IdCountryCurrency   from TransferClosed t  (nolock)    
...
[ValidationFee] money, /*2015-Sep-21*/
[ScannerFee] money	/*2015-Sep-21*/
)

DECLARE @EndDateOP DATETIME = @EndDate-1
 INSERT  INTO #tOtherProd
  EXEC [dbo].[st_GetOtherProductProfitV2] @StartDate, @EndDateOP, null, null, @State
---------------------------------------------- Calculo de Other Products
  ...
  SELECT IdAgent, SUM (DepositAgent) DepositAgent
   ...
   SELECT ab.IdAgent,  
    ISNULL ( ( 
	 ...
	 --INTO #tDepositAgent
     FROM #TempAgents
	INNER JOIN AgentBalance ab ON ab.IdAgent = #TempAgents.IdAgent
	INNER JOIN Agent ag ON  ab.IdAgent = ag.IdAgent and ag.IdAgentBankDeposit not in (42, 43, 46)
...
left join CountryCurrency cc on t.IdCountryCurrency=cc.IdCountryCurrency
left join country c2 on c2.IdCountry=cc.IdCountry
left join Currency cu on cu.IdCurrency=cc.IdCurrency
Order by AgentCode,
...
IF (datediff(millisecond,@initializetime,@endproceduretime) > 2000) BEGIN 
 ...
 VALUES ('st_ReportProfitVCountry', getdate(),Convert(VARCHAR,datediff(millisecond,@initializetime,@endproceduretime))+'ms '+@SUPERTEXT)
END 
