CREATE procedure [dbo].[st_ResponseReturnCodePagosInt]                                              
(                                              
    @IdGateway  int,                                              
...
BEGIN TRY
	...
	--where A.IdGateway=@IdGateway And A.IdGatewayReturnCodeType=@ReturnCodeType And A.ReturnCode=@ReturnCode --#1
                                          
	Insert into [MAXILOG].[dbo].PagosIntResponseLog values (getdate(),@Claimcode,@ReturnCode,@ReturnCodeType,@IdStatusAction,@Description,@XmlValue)                      
	...
	Select @IdTransfer=IdTransfer from ConsecutivoPagosInt with(nolock) where IdConsecutivoPagosInt= CONVERT(int,@Claimcode)  
	...
	Begin                                            
		...
		Begin                                           
			...
			Begin
				exec st_SavePayInfoPagosInt @IdGateway,@IdTransfer,@Claimcode,@XmlValue
			End 
...
Declare @ErrorMessage nvarchar(max)
Select @ErrorMessage=ERROR_MESSAGE()
Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_ResponseReturnCodePagosInt',Getdate(),@ErrorMessage)
END CATCH