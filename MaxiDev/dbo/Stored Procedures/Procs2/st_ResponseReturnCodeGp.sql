CREATE procedure [dbo].[st_ResponseReturnCodeGp]
(                                        
    @IdGateway  int,
...
Insert into GpResponseLog values (getdate(),@Claimcode,@ReturnCode,@ReturnCodeType,@IdStatusAction,@Description,@XmlValue)
...
Begin
	...
	begin
		...
		Begin                     
            exec st_SavePayInfoGp @IdGateway,@IdTransfer,@Claimcode,@XmlValue
        End
...
BEGIN CATCH
	Declare @ErrorMessage nvarchar(max)
	Select @ErrorMessage=ERROR_MESSAGE()
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_ResponseReturnCodeGp',Getdate(),@ErrorMessage)
END CATCH