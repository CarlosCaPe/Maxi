...
-- Description:	Update transfer from hold
-- =============================================
CREATE PROCEDURE [dbo].[st_UpdateTransfer]
...
<log Date="21/04/2020" Author="adominguez">Se ajusta error de ocupaciones</log>
...
BEGIN TRY
    ...
    BEGIN
	   ...
	   ELSE
	   BEGIN
	   IF (LEN(rtrim(LTRIM(@CustomerOccupation))) > 0)
	   Begin
		  IF NOT EXISTS(SELECT 1 FROM DictionaryOccupation with(nolock) WHERE Name = UPPER(rtrim(LTRIM(@CustomerOccupation))) OR NameEs = UPPER(rtrim(LTRIM(@CustomerOccupation))))
			 BEGIN
	...
	*/
	...
	EXEC [dbo].[st_UpdateCustomerInformation]
...
BEGIN CATCH
	...
	INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES('st_UpdateTransfer', GETDATE(), @ErrorMessage);
