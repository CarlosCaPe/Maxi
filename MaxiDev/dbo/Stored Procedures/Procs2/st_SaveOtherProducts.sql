CREATE Procedure [dbo].[st_SaveOtherProducts] (
	@idAgent int,
	@otherProducts XML,
...
Begin Try
	...
	select IdOtherProducts  IdOtherProduct into #otherproductdelete from OtherProducts where IdOtherProducts not in (select IdOtherProduct from #otherproducts)
				...
				if(@IdOtherProduct = 1)
					...
					if(@IdOtherProduct = 5)
						...
						if(@IdOtherProduct not in (1,6,5))
							Begin
								if(exists(select top 1 1 from AgentOtherProductInfo where IdAgent = @idAgent and IdOtherProduct = @IdOtherProduct) )
									Begin
...
Begin Catch                                                
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_SaveOtherProducts',Getdate(),@ErrorMessage)
End Catch 
