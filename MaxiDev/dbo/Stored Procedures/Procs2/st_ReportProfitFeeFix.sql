CREATE procedure [dbo].[st_ReportProfitFeeFix]
(
...
Insert into #TempAgents (IdAgent)
	...
	SUM(case when Fee+AmountInDollars =TotalAmountToCorporate Then AgentCommission else 0 end) over (Partition by t.IdAgent) as AgentcommissionMonthly1,
	SUM(case when Fee+AmountInDollars <>TotalAmountToCorporate Then AgentCommission else 0 end) over (Partition by t.IdAgent) as AgentcommissionRetain1,
    ...
    SUM(case when Fee+AmountInDollars=TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeM1,
    SUM(case when Fee+AmountInDollars<>TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeR1,
	...
	SUM(case when Fee+AmountInDollars =TotalAmountToCorporate Then AgentCommission else 0 end) over (Partition by t.IdAgent) as AgentcommissionMonthly2,
	SUM(case when Fee+AmountInDollars <>TotalAmountToCorporate Then AgentCommission else 0 end) over (Partition by t.IdAgent) as AgentcommissionRetain2,
    ...
    SUM(case when Fee+AmountInDollars=TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeM2,
    SUM(case when Fee+AmountInDollars<>TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeR2,
    ...
    SUM(case when Fee+AmountInDollars=TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeRejM1,
    SUM(case when Fee+AmountInDollars<>TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeRejR1,
    ...
    SUM(case when Fee+AmountInDollars=TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeM2Rej,
    SUM(case when Fee+AmountInDollars<>TotalAmountToCorporate Then ModifierCommissionSlider+ModifierExchangeRateSlider else 0 end) over (Partition by t.IdAgent) as FxFeeR2Rej,
 ...
 --   ChargeDate<@EndDate 
	...
	IncomeFee=IsNull(IncomeFee1,0)+IsNull(IncomeFee2,0)-IsNull(IncomeFeeRej1,0)-IsNull(IncomeFee2Rej,0)-isnull(IncomeFeeCan1,0)-isnull(IncomeFeeCan2,0)/*-IsNull(IncomeFeeCancelLikeReject1,0)-IsNull(IncomeFeeCancelLikeReject2,0)*/, 
