...
CREATE procedure [dbo].[st_SavePayInfoCibanco]   
(
    @IdGateway  int,
    @idTransfer int,
...
begin try
    ...
    select   
        ...
        @BranchCode = upper(dbo.GetValueFromGatewayResponse(@XmlValue,'PayoutBranchCode')),
        @BenIdNumber = upper(dbo.GetValueFromGatewayResponse(@XmlValue,'BeneficiaryIdNumber')),
        @BenIdType = upper(dbo.GetValueFromGatewayResponse(@XmlValue,'BeneficiaryIdType'))
...
begin catch
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_SavePayInfoCibanco: @XmlValue: ' + CONVERT(varchar,@XmlValue),Getdate(),@ErrorMessage)
