create procedure [dbo].[st_ReportProfitConFiltroEstado]
(
...
Insert into #TempAgents (IdAgent)
	Select Distinct t.IdAgent from Transfer t
	...
	where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate and a.AgentState=isnull(@State,a.AgentState)
	Union
	Select t.IdAgent from TransferClosed t
    join agent a with (Nolock) on t.IdAgent=a.idagent
	where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		and DateOfTransfer>@StartDate and DateOfTransfer<@EndDate  and a.AgentState=isnull(@State,a.AgentState)
	Union
    ...
    join agent a with (Nolock) on t.IdAgent=a.idagent
	where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		and DateStatusChange>@StartDate and DateStatusChange<@EndDate and IdStatus in (31,22)	 and a.AgentState=isnull(@State,a.AgentState)
	Union
	Select t.IdAgent from Transfer t
    join agent a with (Nolock) on t.IdAgent=a.idagent
	where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		and DateStatusChange>@StartDate and DateStatusChange<@EndDate and IdStatus in (31,22)	 and a.AgentState=isnull(@State,a.AgentState)	
    Union    
    ...
    join agent a with (Nolock) on t.IdAgent=a.idagent
	where IdCountryCurrency= case when @IdCountryCurrency=0 Then IdCountryCurrency Else @IdCountryCurrency End 
		and DateStatusChange>@StartDate and DateStatusChange<@EndDate and IdStatus in (27)  and a.AgentState=isnull(@State,a.AgentState)
    --union
    --select idagent from #tempSC
    union
     select ab.IDAGENT 
    from 
        agentbalance  ab With (Nolock)
    join 
        AgentOtherCharge oc on ab.idagentbalance=oc.idagentbalance and oc.IdOtherChargesMemo in (6,9,13,19,4,5,11,12,16,17,18,24,25)
	join agent a with (Nolock) on ab.IdAgent=a.idagent
    where 
	...
	join agent a with (Nolock) on ac.IdAgent=a.idagent
	where 
    ...
    Select  SC.IdAgent            
        from [dbo].[SpecialCommissionBalance]  SC
		join agent a with (Nolock) on sc.IdAgent=a.idagent
    WHERE SC.[DateOfApplication]>= @BeginDateSpecialCommission AND SC.[DateOfApplication]<@EndDate
	and a.AgentState=isnull(@State,a.AgentState)
...
select t.IdAgent,AgentCode,AgentName,NumTrans,NumCancel,NumNet,AmountTrans,AmountCancel,AmountNet,CogsTrans,CogsCancel,CogsNet,FxResult,IncomeFee,AgentcommissionMonthly,AgentcommissionRetain,FxFeeM,FxFeeR,isnull(SpecialCommission,0) SpecialCommission,PayerCommission,UnclaimedAmount,UnclaimedCOGS,OtherCharges,OtherChargesC,OtherChargesD,Result,NetResult-isnull(SpecialCommission,0) NetResult,case when NumNet!=0 then (netresult-isnull(SpecialCommission,0))/NumNet else 0 end Margin,isnull(UserName,'') Parent,SalesRep from(
    ...
    (select IdUserSellerParent from Seller where IdUserSeller=IdSalesRep) IdUserSellerParent,
    SalesRep
	from #Result
) t
left join users u on u.iduser=isnull(IdUserSellerParent,0)
