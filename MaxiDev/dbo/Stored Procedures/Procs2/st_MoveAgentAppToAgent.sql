CREATE Procedure [dbo].[st_MoveAgentAppToAgent]
(
...
<ChangeLog>
	...
	<log Date="26/12/2022" Author="maprado">Se agrega AgentBusinessWebsite, ComplianceOfficerPlaceOfBirth, MailCheckTo</log>
</ChangeLog>
********************************************************************/
Set nocount on
...
Else
	Begin
		...
		BEGIN
			EXEC [dbo].[st_AddNoteAgentException] 1, @idAgent, 1, @SystemUser, 'Mark as exception by system; ACH', @HasError, @Message
		END
		/*ELSE
		...
		begin
			...
			exec dbo.st_AgentStatusChange @idAgent, 3, @idUser, 'Suspended by System'
		...
		BEGIN
			EXEC dbo.st_saveAgentChangeHistory @idAgent, @NeedsWFSubaccount,@NeedsWFSubaccountType, @NeedWFSubAccountIdUser, 1;							
		END

		IF (@RequestWFSubaccount = 1)
		BEGIN
			EXEC dbo.st_saveAgentChangeHistory @idAgent, @RequestWFSubaccount, @RequestWFSubaccountType , @RequestWFSubAccountIdUser, 1;
		END 
...
Begin Catch      
 ...
 Insert into dbo.ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_MoveAgentAppToAgent',Getdate(),@ErrorMessage)                                                                                          
End Catch