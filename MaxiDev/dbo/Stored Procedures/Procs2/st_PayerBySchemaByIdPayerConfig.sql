CREATE Procedure [dbo].[st_PayerBySchemaByIdPayerConfig]
(   
    @IdAgent		INT,
    @IdAgentSchema	INT,          
    @IdPaymentType	INT,
...
BEGIN
	...
	select      
	  ...
	  PayerSpreadValue,        
		CASE 
			WHEN IsSwitchSpecExRateGroup = 0 and ISNULL(l.IdSpread ,0)>0 THEN ISNULL((SELECT SD.SpreadValue
			...
			WHEN IsSwitchSpecExRateGroup = 1 AND exists (select s.IdPayer from RefExRateByGroup s where s.IdPayer = IdPayer)  and ISNULL(l.IdSpread ,0)>0 THEN ISNULL((SELECT MAX(SD.SpreadValue)
	   		...
	   		WHEN IsSwitchSpecExRateGroup = 1 AND  not exists (select s.IdPayer from RefExRateByGroup s where s.IdPayer = IdPayer) and ISNULL(l.IdSpread ,0)>0 THEN ISNULL((SELECT SD.SpreadValue
												FROM SpreadDetail SD WITH(NOLOCK)
												WHERE SD.IdSpread = l.IdSpread
													AND case 
															when @IsUSD=1 then @Amount 
	...
	(         
		  ...
		  AND PC.IdPayerConfig = @IdPayerConfig
