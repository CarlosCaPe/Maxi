CREATE PROCEDURE [dbo].[st_InsertSmsFromStatusChange]
...
BEGIN TRY
	...
	SET @InterCode = (SELECT Transfer.IdDialingCodePhoneNumber FROM Transfer WHERE Transfer.IdTransfer = @TransferId)
	...
	IF EXISTS(SELECT 1 FROM [dbo].[StatusToSendCellularMsg] WITH (NOLOCK) WHERE [IdStatus]=@StatusId AND [IdPaymentType]=@IdPaymentType)
		AND EXISTS(SELECT 1 FROM [Infinite].[CellularNumber] WITH (NOLOCK) WHERE [AllowSentMessages] = 1 AND [IsCustomer] = 1 AND [InterCode] = @InterCode AND [NumberWithFormat] = @CelullarNumber)
	BEGIN
		INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES ('st_InsertSmsFromStatusChange', GETDATE(), '01 Va a mandar mensaje')
		...
		INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES ('st_InsertSmsFromStatusChange', GETDATE(), '02 Obtiene mensaje ' + @TextMessage)
		...
		SELECT @TextMessage = replace(@TextMessage, '{{tran_fecha}}', @TransferDate)
				...
				INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES ('st_InsertSmsFromStatusChange', GETDATE(), '03 Mensaje nuevo ' + @TextMessage)
...
BEGIN CATCH
	...
	INSERT INTO [dbo].[ErrorLogForStoreProcedure] ([StoreProcedure], [ErrorDate], [ErrorMessage]) VALUES ('st_InsertSmsFromStatusChange', GETDATE(), @ErrorMessage)
END CATCH
