CREATE Procedure [dbo].[st_SaveBulkDeposit]          
    @deposits XML,                 
    @EnterByIdUser int,     
    @IgnoreWarning bit,     
...
Begin Try 
	...
	Begin     
		...
		Begin     
			...
			if exists(select 1 from AgentDeposit where IdAgent= @IdAgent and DepositDate=@DepositDate and BankName= @BankName
														and Amount= @Amount)
	...
	from @DepositsTemp d 
		inner join Agent a on a.IdAgent= d.IdAgent
...
Begin transaction
	...
	Begin
        ...
        exec st_ApplyDepositMaxicollectionForWeekend @idagent,@maxicollectiondate,@Amount  
