...
CREATE PROCEDURE [dbo].[st_InsertComplianceFormatFaxByTransferId]
...
BEGIN TRY
	...
	DECLARE @Raw NVARCHAR(MAX) = LTRIM(
		...
		+ ',"Note":"Se ha recibido formato de cumplimiento para transacción folio ' + @Folio + '"'
	...
	*/
	...
	+ ISNULL(@Folio,'Error') + ', por favor llene lo que haga falta y envíela firmada por el cliente al (866) 367-6295\n\n'
...
ELSE
BEGIN
INSERT INTO [dbo].[ErrorLogForStoreProcedure] (StoreProcedure,ErrorDate,ErrorMessage)VALUES('[st_InsertComplianceFormatFaxByTransferId]',GETDATE(),'La tranferencia no esta en tabla Transfer, Idtransfer =' + cast(@TransferId as varchar(20))); -- #1
		SET @HasError = 1
...
BEGIN CATCH
	...
	INSERT INTO [dbo].[ErrorLogForStoreProcedure] (StoreProcedure,ErrorDate,ErrorMessage)VALUES('[st_InsertComplianceFormatFaxByTransferId]',GETDATE(),@ErrorMessage) ;
    SET @HasError =1
    SELECT @ResultMessage=[dbo].[GetMessageFromMultiLenguajeResorces] (@IdLenguage,'MESSAGE47')	
END CATCH
