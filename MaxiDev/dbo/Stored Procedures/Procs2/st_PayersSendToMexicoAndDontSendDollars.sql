CREATE PROCEDURE [dbo].[st_PayersSendToMexicoAndDontSendDollars](
   ...
   BEGIN
      ...
      FROM Payer P  
       INNER JOIN   
        ...
        LEFT JOIN RefExRate R1 ON R1.IdCountryCurrency=PC.IdCountryCurrency and R1.Active=1 and R1.RefExRate<>0 and pc.IdGateway=R1.IdGateway and p.IdPayer=R1.IdPayer  
        LEFT JOIN RefExRate R2 ON R2.IdCountryCurrency=PC.IdCountryCurrency and R2.Active=1 and R2.RefExRate<>0 and pc.IdGateway=R2.IdGateway and R2.IdPayer is NULL AND R1.RefExRate IS NULL
        LEFT JOIN RefExRate R3 ON R3.IdCountryCurrency=PC.IdCountryCurrency and R3.Active=1 and R3.IdGateway is NULL and R3.IdPayer is NULL AND R1.RefExRate IS NULL AND R2.RefExRate IS NULL
