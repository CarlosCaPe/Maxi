CREATE Procedure [dbo].[st_SaveAgent](
	@idAgent int,
	...
	if(exists (select top  1 1 from dbo.agent with(nolock) where AgentCode = @agentCode and IdAgent != @idAgent))
		...
		if(@idAgent = 0)
			begin
				...
				EXEC	@return_value = [dbo].[st_GetNewAgentCode]
		...
		begin
			if(@idAgent > 0)
					...
					if(exists (select top  1 1 from dbo.agent with(nolock) where IdAgent = @idAgent))
						begin
							...
							where IdAgent = @idAgent;
							 ...
							 IF ISNULL(@enteredByIdUser, 0) = 0
							 BEGIN
								INSERT INTO Soporte.InfoLogForStoreProcedure(StoreProcedure, InfoDate, InfoMessage, ExtraData) VALUES('st_SaveAgent', GETDATE(), 'Usuario null o 0 en stored procedures',  '@EnterByIdUser = ' + CONVERT(VARCHAR(10), @enteredByIdUser))
							 END

							EXEC dbo.st_saveAgentChangeHistory @idAgent, @agentPhone, @agentPhoneType, @enteredByIdUser, 0;
							EXEC dbo.st_saveAgentChangeHistory @idAgent, @agentFax, @agentFaxType, @enteredByIdUser, 0;
							EXEC dbo.st_saveAgentChangeHistory @idAgent, @agentEmail, @agentEmailType, @enteredByIdUser, 0;
							EXEC dbo.st_saveAgentChangeHistory @idAgent, @agentClass, @agentClassType, @enteredByIdUser, 0;
							-- History of Wells Fargo Needs & Request

							EXEC dbo.st_saveAgentChangeHistory @idAgent, @NeedsWFSubaccount,@NeedsWFSubaccountType, @enteredByIdUser, 0;							
							EXEC dbo.st_saveAgentChangeHistory @idAgent, @RequestWFSubaccount, @RequestWFSubaccountType , @enteredByIdUser, 0;
				...
				begin
					...
					EXEC dbo.st_saveAgentChangeHistory @IdAgentOut, @agentPhone, @agentPhoneType, @enteredByIdUser, 0;
					EXEC dbo.st_saveAgentChangeHistory @IdAgentOut, @agentFax,	@agentFaxType,	@enteredByIdUser, 0;
					EXEC dbo.st_saveAgentChangeHistory @IdAgentOut, @agentEmail, @agentEmailType, @enteredByIdUser, 0;
					EXEC dbo.st_saveAgentChangeHistory @idAgent,	@agentClass, @agentClassType, @enteredByIdUser, 0;
			...
			BEGIN
				EXEC [dbo].[st_AddNoteAgentException] 1, @IdAgentOut, 1, @SystemUser, 'Mark as exception by system; ACH', @HasError, @MessageOut
			END
			/*ELSE
			...
			BEGIN
				...
				EXEC dbo.st_saveAgentChangeHistory @IdAgentOut, @PhoneNumber, @agentAdditionalPhoneType, @enteredByIdUser, 0;
...
Begin Catch
	...
	Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_SaveAgent',Getdate(),@ErrorMessage)                                                
