CREATE procedure [dbo].[st_SaveFeeChecks]
(
    @IdFee int, 
    @IdAgent int,
	@AllowChecks bit,
...
SELECT @CanSetChecksInfo =  CASE WHEN Patindex('%CH%',[Action]) > 0 THEN 1 ELSE 0 END FROM OptionUsers with(nolock) WHERE IdOption =8 AND  IdUser =@EnterByIdUser
...
BEGIN CATCH
 ...
 Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_SaveFeeChecks',Getdate(),@ErrorMessage)    
END CATCH
