...
<ChangeLog>
<log Date="06/07/2018" Author="azavala">Creacion</log>
<log Date="29/11/2018" Author="jdarellano" Name="#1">Se modifica filtro a solicitud de cliente, para que tome en cuenta cheques en estatus "Stand By" y "Pending Gateway Response" (Ticket 1009).</log>
<log Date="04/09/2019" Author="jdarellano" Name="#2">Se agrega "Inner Join" con tabla "Operation.ProductTransfer" para filtrado de registros(Ticket 2013).</log>
</ChangeLog>
*********************************************************************/
CREATE procedure [dbo].[st_ReportCollectionControl]  
@BeginDate datetime,                        
@EndDate datetime                        
as                        
...
--SELECT DISTINCT TYPEOFMOVEMENT FROM AgentBalance                 
   ...
   Join #temp15 B on (A.DateOfMovement=B.DateOfMovement and A.IdAgent=B.IdAgent)                
    where not (A.TypeOfMovement='CGO' and ( A.Description like '% Retransfer Credit - Folio:%' or  A.Description like '% Oklahoma State Fee - Folio:%' ) )               
