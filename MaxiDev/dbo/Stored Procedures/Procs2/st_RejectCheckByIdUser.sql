...
CREATE procedure [dbo].[st_RejectCheckByIdUser]
(
--declare
    @IdCheck int ,--= 15076,
    @EnterByIdUser int  ,--=1,
...
BEGIN TRY
	...
	EXEC checks.st_CheckCancelToAgentBalance @IdCheck,@EnterByIdUser,1;

	EXEC [dbo].[InsCheckRejectHistory] @IdCheck,@EnterByIdUser, @Note

	IF @Note LIKE '%Closed Account%'
	Begin
		DECLARE @ReturnDate datetime = getdate()
		EXECUTE [Checks].[st_InsertDenyListIssuerChecks] @EnterByIdUser, @IdCheck, @ReturnDate, '', 0
	End
...
BEGIN CATCH
	...
	INSERT INTO ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_RejectCheckByIdUser',Getdate(),@ErrorMessage)                                                                                            
END CATCH
