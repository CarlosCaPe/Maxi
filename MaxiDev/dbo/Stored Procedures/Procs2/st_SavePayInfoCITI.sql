...
CREATE procedure [dbo].[st_SavePayInfoCITI]   
(
    @IdGateway  int,
    @idTransfer int,
...
begin try
    ...
    select   
        @dateofpayment =  convert(datetime,dbo.GetValueFromGatewayResponse(@XmlValue,'Date_Pago_Invioce')),
        @BranchCode = '',
        @BenIdNumber = '',
        @BenIdType = ''
...
begin catch
    ...
    Insert into ErrorLogForStoreProcedure (StoreProcedure,ErrorDate,ErrorMessage)Values('st_SavePayInfoCITI: @XmlValue: ' + CONVERT(varchar,@XmlValue),Getdate(),@ErrorMessage)
